...
add('chromium-bootstrap-isolates-irrelevant-google-apt-repo', hasAll(workflow, ['dl.google.com/linux/chrome-stable/deb', '${source_file}.ywi-disabled', 'npx playwright install --with-deps chromium']) && workflow.indexOf('dl.google.com/linux/chrome-stable/deb') < workflow.indexOf('npx playwright install --with-deps chromium'), 'The transient CI runner disables the unrelated Google Chrome APT source before Playwright resolves Ubuntu dependencies, preventing third-party index drift from skipping the mandatory browser tail.');
