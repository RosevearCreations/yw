...
add('production-supabase-network-is-blocked', hasAll(browser, ["page.route('https://jmqvkgiqlimdhcofwkxr.supabase.co/**'", 'productionSupabaseRequests += 1', 'expect(getProductionSupabaseRequests()).toBe(0)']), 'Deterministic CI both intercepts and fails if the canonical shell attempts a Production Supabase request.');
...
add('actual-shell-boundaries-rendered', hasAll(browser, ["page.locator('h1')", "#mainNav a[data-module]", "#operationsCockpit", 'scrollWidth > window.innerWidth + 1']), 'Single-H1, four-module shell, staff-cockpit isolation, and horizontal overflow are browser-enforced.');
