...
add('approval-is-the-only-public-copy-path', hasAll(operations, ['async function publishApprovedAsset', "from('public-assets').upload", "action === 'visual_asset_register' || action === 'visual_asset_decision'", "status === 'approved' && !sourceUrl && existing", 'publication = await publishApprovedAsset(supabase, existing)']), 'Public asset copies remain behind the protected visual_asset_decision approval path.');
