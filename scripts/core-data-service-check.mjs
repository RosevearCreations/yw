...
const helper = read('supabase/functions/_shared/core-data-read-models.ts');
const endpoint = read('supabase/functions/core-data-read/index.ts');
...
add('endpoint-auth-required', hasAll(endpoint, ['supabase.auth.getUser(token)', "select('id,role,staff_tier,is_active')", "actorProfile.is_active !== true"]), 'Endpoint requires a valid active signed-in profile.');
...
add('browser-no-write-api', !/\b(create|save|update|delete|remove|upsert)\s*[:(]/i.test(browser) && hasAll(browser, ['read,', 'readEntity,']), 'Browser surface exposes reads/invalidation only.');
