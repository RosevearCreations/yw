...
add('operations-schema155-live-update-actions', all(operations, ["const SCHEMA = 158", 'work_order_live_update_create', 'work_order_live_update_retract', 'customer_notification_retry', 'ywi_rpc_enqueue_customer_live_update_notification', 'v_work_order_live_update_queue']), 'Operations action path is wired.');
add('portal-schema155-live-update-filter', all(portal, ["const SCHEMA = 158", 'portalLiveUpdates', 'portalNotificationPreference', "action === 'set_live_update_notifications'", 'v_customer_portal_live_updates', 'live_updates: liveUpdates']), 'Portal stays constrained to published customer-safe updates.');
