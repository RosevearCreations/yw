#!/usr/bin/env node
/*
  Schema 154 staging proof harness.
...
if(!live){ console.log('\nSKIP live staging proof — set YWI_RUN_STAGING_RPC_TESTS=1 only after schema 154 is deployed to a dedicated non-production project.'); process.exit(0); }
...
const runKey=`staging-schema154-${new Date().toISOString().replace(/[:.]/g,'-')}-${Math.random().toString(16).slice(2,8)}`;
const created=await rest('operations_staging_test_runs',{method:'POST',body:JSON.stringify({run_key:runKey,environment_label:'staging',suite_name:'operations_rpc_e2e_schema154',run_status:'started',requested_by_profile_id:actorId,summary:{build:'2026-06-30a',schema:154,fixture_mode:process.env.YWI_STAGING_CREATE_FIXTURES==='1'}})});
...
let fixture=null;
await liveCase('schema_drift_is_current',async()=>{const rows=await rest('v_schema_drift_status?select=*');const row=rows?.[0]||{};if(Number(row.latest_applied_schema_version)<154||row.drift_status!=='current')throw new Error(`Expected schema 154 current: ${JSON.stringify(row)}`);return row;});
...
await liveCase('release_readiness_dashboard_view',async()=>{const rows=await rest('v_release_readiness_dashboard?select=staging_evidence_status,public_content_status,policy_ready,backup_rehearsal_status');const row=rows?.[0]||{};if(!row.staging_evidence_status)throw new Error('Schema 154 release dashboard is unavailable.');return row;});
await liveCase('capability_snapshot_has_release-proof-actions',async()=>{const result=await rest('rpc/ywi_get_operations_capabilities',{method:'POST',body:JSON.stringify({p_actor_profile_id:actorId})});const snap=Array.isArray(result)?result[0]:result;if(!snap?.actions?.staging_fixture_create||!snap?.actions?.content_signal_record||!snap?.actions?.release_readiness_snapshot)throw new Error('Schema 154 capability actions missing.');return {role:snap.actor_role,rank:snap.actor_rank};});
...
try{await rest('operations_staging_test_results',{method:'POST',body:JSON.stringify(cases.map((item)=>({...item,run_id:run.id})))});await rest(`operations_staging_test_runs?id=eq.${encodeURIComponent(run.id)}`,{method:'PATCH',body:JSON.stringify({run_status:failed.length?'failed':'passed',finished_at:new Date().toISOString(),summary:{build:'2026-06-30a',schema:154,case_count:cases.length,failed_count:failed.length,fixture_set_id:fixture?.fixture_set_id||null}})});}catch(error){console.error(`WARN  Could not record full test outcome: ${error instanceof Error?error.message:String(error)}`);}
