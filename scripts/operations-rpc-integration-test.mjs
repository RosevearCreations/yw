#!/usr/bin/env node
/**
 * Transactional operation and schema 158 wiring check.
...
add('operations-schema155-live-update-wiring', hasAll(operations, ['const SCHEMA = 158', 'work_order_live_update_create', 'work_order_live_update_retract', 'ywi_rpc_create_work_order_live_update', 'v_work_order_live_update_queue']), 'Operations Edge Function uses live-update RPCs and queue.');
add('portal-schema155-live-update-filter', hasAll(portal, ['const SCHEMA = 158', 'portalLiveUpdates', 'v_customer_portal_live_updates', 'live_updates: liveUpdates']), 'Portal is limited to the portal-safe update view.');
...
try {
  ...
  if (Number(schemaRow.latest_applied_schema_version) < 156 || schemaRow.drift_status !== 'current') throw new Error(`Schema 158 is not current: ${JSON.stringify(schemaRow)}`);
