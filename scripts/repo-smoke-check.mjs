...
add('schema178-dynamic-dependency-preflight',schema178.includes('required_by_schema <= coalesce((select expected_schema_version from public.v_schema_drift_status limit 1),178)')&&!/where\s+required_by_schema\s*<=\s*173\b/i.test(schema178),'Admin schema-preflight dependency coverage follows the current schema marker.');
