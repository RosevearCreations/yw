#!/usr/bin/env node
/** Repository-level static sanity check for build 2026-09-01a / schema 159. */
...
add('active-readme-current', hasAll(readme, ['2026-09-01a', 'schema `159`', 'Safety / OHSA', 'Finance', 'Jobs', 'Admin']), 'README identifies schema 159 modular release.');
add('handoff-docs-current', hasAll(handbook, ['Schema target:', '`159`', 'Module permissions', 'Safety / OHSA']) && hasAll(nextSteps, ['Schema:', '`159`', 'Safety-only acceptance test', 'Direct API bypass test']), 'Active handoff explains current controls and tests.');

// Schema/reference integrity.
const migrationBlocks = (schema.match(/BEGIN MIGRATION:/g) || []).length;
add('canonical-schema-through-159', schema.includes('BEGIN MIGRATION: 030_') && schema.includes('BEGIN MIGRATION: 159_module_boundaries_permission_gated_navigation') && migrationBlocks === 130, `Canonical schema has ${migrationBlocks} migration blocks.`);
...
add('build-cache-marker-current', hasAll(index, ['2026-09-01a', 'operations-cockpit.js?v=2026-09-01a']) && read('server-worker.js').includes('ywi-shell-v2026-09-01a'), 'HTML and service-worker cache marker are current.');
add('route-generator-cache-marker-current', routeGenerator.includes("const build = '2026-09-01a';"), 'Generated public pages reference current static build marker.');
...
for (const file of [
  ...
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'scripts/module-permissions-check.mjs', 'tests/browser/operations-portal.spec.mjs',
...
const jsFiles = [
  ...
  'scripts/release-readiness-dashboard-check.mjs', 'scripts/live-work-updates-check.mjs', 'scripts/customer-notification-delivery-check.mjs',
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'scripts/module-permissions-check.mjs', 'scripts/repo-smoke-check.mjs', 'playwright.config.mjs', 'app.js', 'server-worker.js'
];
...
for (const script of [
  ...
  'scripts/release-readiness-dashboard-check.mjs', 'scripts/live-work-updates-check.mjs', 'scripts/customer-notification-delivery-check.mjs',
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'scripts/module-permissions-check.mjs', 'scripts/operations-rpc-staging-e2e.mjs'
]) {
