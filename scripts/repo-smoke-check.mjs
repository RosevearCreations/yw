#!/usr/bin/env node
/** Repository-level static sanity check for build 2026-08-05a / schema 158. */
...
add('retired-markdown-archive-present', exists('archive/retired-markdown-2026-08-05a') && walk(path.join(root, 'archive/retired-markdown-2026-08-05a')).filter((file) => file.endsWith('.md')).length >= 80, 'Historical Markdown is preserved in dated archive.');
...
add('active-readme-current', hasAll(readme, ['2026-08-05a', 'schema `158`', 'supervisor closeout', 'customer signoff', 'invoice readiness']), 'README identifies schema 158 release.');
add('handoff-docs-current', hasAll(handbook, ['Schema target:', '158', 'supervisor closeout', 'app-shell navigation correction']) && hasAll(nextSteps, ['Schema:', '158', 'top-menu navigation test', 'invoice readiness']), 'Active handoff explains current controls and tests.');
...
add('portal-schema158-wiring', hasAll(portal, ["const BUILD = '2026-08-05a'", 'const SCHEMA = 158', 'portalLiveUpdates', 'portalNotificationPreference', 'portalExecutionProofs', 'portalCloseoutPackages', "action === 'sign_closeout'", 'ywi_rpc_customer_sign_work_order_closeout']), 'Customer portal returns safe updates/proofs/closeouts and signs through RPC.');
add('dispatcher-protected', hasAll(dispatcher, ["const BUILD = '2026-08-05a'", 'const SCHEMA = 158', 'YWI_CUSTOMER_NOTIFICATION_DELIVERY_ENABLED', 'YWI_CUSTOMER_NOTIFICATION_RUN_TOKEN', "'Idempotency-Key'", "p_result_status:'manual_review'"]), 'Dispatcher requires run token and safe provider uncertainty handling.');
add('upload-private-review-asset', hasAll(upload, ["const BUCKET = 'review-assets'", "const BUILD = '2026-08-05a'", 'const SCHEMA = 158', 'review_only:true', 'imageDimensions', 'checksum_sha256']), 'Upload handler verifies and stores review media privately.');
add('webhook-marker-current', hasAll(webhook, ["const BUILD='2026-08-05a'", 'const SCHEMA=158', 'validateSession', 'ywi_rpc_record_portal_deposit_paid', 'ywi_refresh_stripe_webhook_alerts']), 'Webhook retains verified payment and alert logic.');
add('accountant-marker-current', hasAll(accountant, ["const BUILD = '2026-08-05a'", 'const SCHEMA = 158', 'ywi_rpc_capture_accountant_close_snapshot', 'createSignedUrl']), 'Accountant export retains private close/mapping handling.');
...
add('build-cache-marker-current', hasAll(index, ['2026-08-05a', 'operations-cockpit.js?v=2026-08-05a']) && read('server-worker.js').includes('ywi-shell-v2026-08-05a'), 'HTML and service-worker cache marker are current.');
add('route-generator-cache-marker-current', routeGenerator.includes("const build = '2026-08-05a';"), 'Generated public pages reference current static build marker.');
...
for (const file of [
  ...
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'tests/browser/operations-portal.spec.mjs',
...
const jsFiles = [
  ...
  'scripts/release-readiness-dashboard-check.mjs', 'scripts/live-work-updates-check.mjs', 'scripts/customer-notification-delivery-check.mjs',
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'scripts/repo-smoke-check.mjs', 'playwright.config.mjs', 'app.js', 'server-worker.js'
];
...
for (const script of [
  ...
  'scripts/release-readiness-dashboard-check.mjs', 'scripts/live-work-updates-check.mjs', 'scripts/customer-notification-delivery-check.mjs',
  'scripts/service-execution-proof-check.mjs', 'scripts/supervisor-closeout-check.mjs', 'scripts/navigation-route-priority-check.mjs', 'scripts/operations-rpc-staging-e2e.mjs'
]) {
