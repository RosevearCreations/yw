...
const processorSection=migration.split('create or replace function public.ywi_finance_process_job_completed_event')[1]?.split('revoke all on function public.ywi_finance_process_job_completed_event')[0] || '';
const runnerSection=migration.split('create or replace function public.ywi_finance_run_job_completion_consumer')[1]?.split('revoke all on function public.ywi_finance_run_job_completion_consumer')[0] || '';
const executableSection=`${processorSection}\n${runnerSection}`;
const failures=[];
...
check('schema171-event-contract-gate',runnerSection.includes("event_key='jobs.job_completed'")&&runnerSection.includes("'finance'=any(c.consumer_modules)")&&runnerSection.includes('contract_version=1'));
check('schema171-schema-current-gate',runnerSection.includes('s.expected_schema_version >= 171')&&runnerSection.includes('s.latest_applied_schema_version >= 171')&&runnerSection.includes("s.drift_status='current'"));
check('schema171-per-event-failure-isolation',runnerSection.includes('get stacked diagnostics')&&runnerSection.includes('on conflict(source_event_id) do update')&&runnerSection.includes('v_failed := v_failed + 1'));
check('schema171-run-status-proof',runnerSection.includes("'completed_with_failures'")&&runnerSection.includes("run_status='failed'"));
check('schema171-review-only',processorSection.includes("'queue_review'")&&!executableSection.includes("'create_invoice_candidate'")&&!executableSection.includes("'create_journal_candidate'")&&!executableSection.includes("'mark_posted'"));
check('schema171-no-jobs-state-writeback',!/update\s+public\.(?:jobs|work_orders|job_completion_reviews)\b/i.test(executableSection));
check('schema171-no-scheduler',!/cron\.|pg_cron|schedule\s+/i.test(executableSection));
