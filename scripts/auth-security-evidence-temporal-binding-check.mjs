...
function fetchHarness({artifactCreatedAt=ARTIFACT_CREATED_AT,candidate=makeCandidate()}={}){
  ...
  const fetchImpl=async (url,options={})=>{
    ...
    if(url===ARTIFACTS_URL)return {ok:true,status:200,json:async()=>({total_count:1,artifacts:[artifact(artifactCreatedAt)]}),text:async()=>''};
...
add('valid-record-path-temporally-verified',recorded.ok && recorded.write_performed && recorded.auth_capture_temporal_verified===true && recorded.auth_capture_temporal_observed_at===OBSERVED_AT && recorded.auth_capture_temporal_artifact_created_at===ARTIFACT_CREATED_AT);
...
try{
  ...
  refreshedRejected=String(error?.message || error).includes('later than the verified artifact creation window');
}
...
try{
  ...
  earlyRejected=String(error?.message || error).includes('too old relative');
}
add('artifact-lag-over-15-minutes-blocks-before-supabase',earlyRejected && !earlySupabaseCalled);

for(const item of checks)console.log(`${item.ok?'PASS':'FAIL'}  ${item.name}`);
const failed=checks.filter((item)=>!item.ok);
console.log(`\n${checks.length-failed.length}/${checks.length} Build 268 Auth temporal provenance checks passed.`);
