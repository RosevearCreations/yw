...
export function evaluateStagingTarget(env={}){
  ...
  if(expectedRef && expectedRef===productionRef)errors.push('YWI_STAGING_PROJECT_REF must not equal the Production project ref.');
  if(actualRef && actualRef===productionRef)errors.push('SUPABASE_URL resolves to the Production project and is forbidden for staging proof.');
  ...
  return {
    ...
    production_project_ref:productionRef || KNOWN_PRODUCTION_PROJECT_REF,
    ...
    non_production_target:Boolean(actualRef && expectedRef && actualRef!==productionRef && expectedRef!==productionRef),
