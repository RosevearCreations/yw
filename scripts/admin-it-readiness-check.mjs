...
const runtimeMigration=read('sql/207_runtime_readiness_load_convergence.sql');
...
check('schema207-migration-present',exists('sql/207_runtime_readiness_load_convergence.sql'));
check('schema207-lightweight-finance-view',runtimeMigration.includes('v_it_finance_runtime_readiness_status')&&runtimeMigration.includes('persisted_release_proof_plus_live_controls'));
check('schema207-interactive-view-does-not-run-finance-assertions',runtimeMigration.includes("pg_get_viewdef('public.v_it_finance_runtime_readiness_status'::regclass,true)")&&runtimeMigration.includes("not like '%ywi_finance_release_hardening_assertions%'"));
check('schema207-open-rail-rewired',runtimeMigration.includes('from public.v_it_finance_runtime_readiness_status')&&!/finance_state as \([\s\S]{0,500}from public\.v_it_finance_release_hardening_status/.test(runtimeMigration));
check('schema207-business-rails-stay-open',runtimeMigration.includes("count(*) from public.v_it_open_rail_acceptance_readiness where rail_status<>'complete')=11"));
check('schema207-finance-provider-off',runtimeMigration.includes('execution_enabled=true or provider_mutation_enabled=true'));
check('schema207-marker',runtimeMigration.includes("207,'207_runtime_readiness_load_convergence'")&&runtimeMigration.includes('select 207 as expected_schema_version'));
...
check('runtime-readiness-endpoint-present',exists('supabase/functions/admin-it-readiness-runtime/index.ts'));
check('runtime-readiness-admin-only',runtimeEndpoint.includes('normalizeRole(actorProfile.role) !== "admin"')&&runtimeEndpoint.includes('Active Admin role is required'));
check('runtime-readiness-bounded-sources', ['v_schema_drift_status','v_it_release_authority_status','v_it_scorecard_progress_truth_status','v_it_open_rail_acceptance_readiness','v_admin_module_access_integrity','v_admin_error_health_center','v_admin_function_readiness_checks','v_it_current_admin_todo'].every((value)=>runtimeEndpoint.includes(value)));
check('runtime-readiness-no-deep-rpc',!runtimeEndpoint.includes('.rpc(')&&!runtimeEndpoint.includes('ywi_finance_release_hardening_assertions'));
check('runtime-readiness-deferred-truth',runtimeEndpoint.includes('deep_verification_deferred: true')&&runtimeEndpoint.includes('explicit_deep_verification_only'));
check('runtime-readiness-source-errors-red',runtimeEndpoint.includes('sourceErrors.length')&&runtimeEndpoint.includes('criticalBlocking > 0 ? "red"'));
check('runtime-readiness-jwt-config',/\[functions\.admin-it-readiness-runtime\][\s\S]*?verify_jwt\s*=\s*true/.test(config));
...
check('it-ui-bounded-readiness-endpoint',itUi.includes("jsonFetch?.('admin-it-readiness-runtime'")&&itUi.includes("action:'it_readiness_runtime'")&&itUi.includes('timeoutMs:12000'));
