...
add('schema190-no-extension-relocation',!/\balter\s+extension\b/i.test(migration));
add('schema190-no-auth-mutation',!/\b(?:update|insert\s+into|delete\s+from)\s+auth\./i.test(migration));
add('schema190-no-business-row-rewrite',!/(?:update|delete\s+from|insert\s+into)\s+public\.(?:jobs|quotes|customers|submissions|invoices|payments|financial_transactions|bank_transactions)\b/i.test(migration));
...
add('schema190-release-switches-closed',!/(execution_release_enabled|provider_mutation_enabled)\s*=\s*true/i.test(migration));
