#!/usr/bin/env node
/**
 * Schema 155 live-work-update contract.
...
add('operations-schema155-wiring', hasAll(operations, ["const SCHEMA = 155","v_work_order_live_update_queue","work_order_live_update_create","work_order_live_update_retract","ywi_rpc_create_work_order_live_update","ywi_rpc_retract_work_order_live_update"]), 'Operations Edge Function fetches and writes live updates through RPCs.');
add('portal-schema155-private-filter', hasAll(portal, ["const SCHEMA = 155","portalLiveUpdates","v_customer_portal_live_updates","live_updates: liveUpdates"]), 'Token portal returns only portal-safe update view data.');
add('cockpit-live-update-ui', hasAll(cockpit, ['Schema 155 field-to-customer update controls','oc_live_update_form','data-oc-work-order-select','data-oc-live-update-assets','renderLiveUpdateQueue','job-update-retract']), 'Cockpit supports explicit staff/customer update entry and retraction.');
...
if (!url || !key) {
  console.log('\nSKIP live schema 155 read — no staging service-role credentials supplied.');
  process.exit(0);
}
