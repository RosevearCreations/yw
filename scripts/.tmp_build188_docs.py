...
section="""## Build 188 final release authority
...
"""
current_block="""**Current live authority:** Schema `188 / 188` — **Build 188 COMPLETE**  
...
for p in files:
    s=p.read_text()
    if '## Build 188 final release authority' not in s:
        first=s.find('\n')
        s=s[:first+1]+'\n'+section+s[first+1:]
    s=s.replace('These historical anchors remain audit evidence only; current live authority is Build 187 / Schema 187 with exact-main release proof recorded.',
    ...
    s=re.sub(r'\*\*Current live (?:schema )?authority:\*\*.*?(?=\n\n)',current_block,s,count=1,flags=re.S)
    if '- Schema 188 / Build 188 — **COMPLETE**' not in s:
        s=s.replace('- Schema 187 / Build 187 — **COMPLETE**: six-rail staging scenario catalog, pending-case seeding, prerequisite truth, explicit human case evidence, fail-closed finalization, and exact-main release authority.',
    ...
    if p.name=='NEXT_STEPS_AND_SANITY_CHECK.md':
...
"""
        ...
        if '- **Build 188 — Open-rail acceptance readiness and current-action truth — COMPLETE**' not in s:
            s=s.replace('- **Build 187 — Staging acceptance scenario catalog + prerequisite truth convergence — COMPLETE**',
                        '- **Build 187 — Staging acceptance scenario catalog + prerequisite truth convergence — COMPLETE**\n- **Build 188 — Open-rail acceptance readiness and current-action truth — COMPLETE**')
        s=s.replace('## Scorecard after Build 187','## Scorecard after Build 188')
        s=s.replace('41 total rails;\n- 30 complete / 11 open;','42 total rails;\n- 31 complete / 11 open;')
    s=s.replace('`admin-it-control` v14 ACTIVE / JWT enabled','`admin-it-control` v15 ACTIVE / JWT enabled')
    p.write_text(s)
