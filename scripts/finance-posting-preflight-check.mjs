...
const ui=fs.readFileSync('js/finance-ui.js','utf8');
...
assert.ok(ui.includes('Posting preflight'),'Finance UI must surface posting preflight.');
assert.ok(ui.includes("finance-job-completion-posting-approval"),'Finance UI must use the protected posting-approval/preflight boundary.');
assert.ok(ui.includes("action:'preflight'")||ui.includes('action: \'preflight\''),'Finance UI must support explicit read-only preflight.');
assert.ok(!/ar_invoices|gl_journal_batches/.test(ui),'Browser must not write or directly target AR/GL tables.');
