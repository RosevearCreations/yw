...
for(const [name,text] of [['handbook',handbook],['next-steps',nextSteps]]){
  for(const value of [
    '.github/workflows/auth-security-evidence-capture.yml',
    '.github/workflows/auth-security-evidence-authorized-record.yml',
    'capture_run_id',
    ...
    'one control per dispatch',
    'I.T. Readiness',
  ])assert.ok(text.includes(value),`${name} must retain Auth evidence operator guidance: ${value}`);
  assert.equal(/Build\s+\d+|Run\s+#?\d+|[0-9a-f]{40}/i.test(text),false,`${name} must remain current operating authority, not a historical build/run/SHA ledger.`);
}
