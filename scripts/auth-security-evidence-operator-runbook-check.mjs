...
const runbook=fs.readFileSync('docs/AUTH_SECURITY_EVIDENCE_OPERATOR_RUNBOOK.md','utf8');
...
const requiredRunbook=[
  ...
  'I.T. & System',
  'Access & Security',
  'does not enable or disable an Auth control',
];
for(const value of requiredRunbook)assert.ok(runbook.includes(value),`Operator runbook must retain: ${value}`);
assert.equal(/Build\s+\d+|Run\s+#?\d+|[0-9a-f]{40}/i.test(runbook),false,'Operator runbook must remain current operating authority, not a historical build/run/SHA ledger.');
...
for(const value of [
  'AUTH_SECURITY_EVIDENCE_OPERATOR_RUNBOOK.md',
  '.github/workflows/auth-security-evidence-capture.yml',
  '.github/workflows/auth-security-evidence-authorized-record.yml',
  'capture_run_id',
  'capture_run_attempt',
  ...
  'one control per dispatch',
  'I.T. Readiness',
])assert.ok(handbook.includes(value),`Handbook must retain Auth evidence operator guidance: ${value}`);
