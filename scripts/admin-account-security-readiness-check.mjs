...
add('runtime-load-cache-invalidated', serviceWorker.includes("const CACHE_NAME = 'ywi-shell-v2026-09-07a';") && serviceWorker.includes("fetch(assetUrl, { cache: 'reload' })") && serviceWorker.includes("'/js/workspace-organization.js'"), 'Service worker cache generation forces the bounded readiness runtime and current workspace organization into the active shell.');
