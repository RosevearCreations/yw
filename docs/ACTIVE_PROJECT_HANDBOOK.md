...
The preferred operator path is documented in `docs/AUTH_SECURITY_EVIDENCE_OPERATOR_RUNBOOK.md`. Manually run `.github/workflows/auth-security-evidence-capture.yml` from canonical `main`, retain its exact `capture_run_id` and `capture_run_attempt`, independently confirm the capture is genuine official Supabase Management API evidence, then manually run `.github/workflows/auth-security-evidence-authorized-record.yml`. Recording is **one control per dispatch**: select either leaked-password protection or MFA, supply the exact capture run/attempt, and use `I_CONFIRM_AUTH_EVIDENCE_RECORD` plus `I_CONFIRM_OFFICIAL_SUPABASE_SOURCE` only after the source-authenticity decision is complete. Repeat the recording workflow separately for the second control when required, then refresh **I.T. Readiness** to confirm the freshness-aware current authority. Capture and recording remain manually separated; neither workflow may automatically chain into the other.
