...
## Customer notification delivery contract

The customer notification delivery path for customer-visible service updates is consent-first and fail-closed. Email delivery requires an **explicit opt-in** stored through the protected customer-portal preference action; lack of consent means no delivery attempt. The notification outbox, delivery attempts and claim/complete/retry RPCs remain service-private, while the staff queue exposes only the bounded delivery-review fields needed for Operations and does not expose customer email addresses or portal tokens.

The dispatcher remains disabled unless its delivery-enable guard and run token are deliberately configured. Provider calls use an idempotency key. Transport uncertainty must move to **manual review** rather than being treated as delivered or automatically retried, and staff retry remains an explicit reviewed action. Customer emails may point back to the secure portal but must not include staff-only notes, private images, access details, internal costs or margin context.

Phone acceptance must keep the customer preference usable with touch-friendly controls and no overflow. Desktop acceptance must keep pending-consent, delivered and manual-review states readable while preserving the safe queue boundary. Historical notification migration metadata remains audit history; current release/schema authority is derived separately from the current schema authority.

## Auth security evidence contract

Leaked-password protection and MFA are Supabase Auth control-plane settings and therefore require **recent authoritative external evidence** before either follow-up can be treated as secure. Authoritative evidence must identify the actual current setting from the **Supabase Dashboard** or **Supabase Management API**, retain a durable evidence reference, and match the secure state for that control. Manual notes, screenshots or other supporting context may be retained, but a row labelled `manual_external` is non-authoritative and cannot produce `verified_secure`; Supabase advisor rows, historical advisor snapshots, database catalogs, and the absence of an advisor warning are also not sufficient proof. Auth security evidence is service-private, freshness-aware, and remains separate from application release authority; source changes do not mutate these external Auth settings or auto-close their Current Admin To-Do items.
...
`npm run test:repo` verifies repository hygiene and durable documentation rules. `npm run test:it` verifies I.T. authority, including current-readiness versus historical-audit separation. `npm run test:auth-security-evidence` verifies the Auth security evidence authority, freshness rules, Supabase Dashboard/Management API provenance, non-authoritative manual/advisor evidence, Current Admin To-Do derivation, and safety invariants. `npm run test:next-safe-action` verifies current To-Do parity, safe staging-candidate classification, external-evidence separation, blocked accounting behavior, durable guidance, and preservation of Finance/provider and human-rail boundaries. `npm run test:staging-environment-guard` verifies Production denial, explicit staging enablement, endpoint/UI lock behavior, durable guidance, and preservation of human-gated rails. `npm run test:current-schema-staging-runbook` verifies exact current-schema runbook guidance, historical catalog preservation, next-action safety, and fail-closed schema drift semantics. `npm run test:core-live-write-staging` verifies the guarded quote/contact runner, exact disposable-data cleanup, public-contract invocation, retained human review, and Production refusal. `npm run test:job-lifecycle` plus `npm run test:browser:job-lifecycle` permanently enforce the live update, execution proof and closeout chain across phone, desktop and customer-safe portal surfaces. `npm run test:notifications`, `npm run test:notification-enforcement`, and `npm run test:browser:notifications` permanently enforce customer notification consent, outbox/delivery/retry safety, fail-closed provider dispatch, phone preference usability, desktop delivery review and customer-safe content. `npm run test:help-seo` verifies Help/search/H1/static search controls. `npm run test:search-discovery` verifies canonical/sitemap parity, freshness, structured data, fail-closed conflicts, and the no-automatic-submission boundary. Rendered browser acceptance includes phone and desktop Help/app/public-page layout checks plus canonical-host, canonical-conflict, noncanonical-preview indexing behavior, and staging environment lock behavior.