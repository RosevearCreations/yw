...
- Admin staff creation can appear frozen when the browser can reach the site shell but the direct `admin-manage` Edge Function call stalls. This pass added a same-origin compatibility fallback, but the remaining risk is environment drift if the deploy lacks the new `/api/auth/admin-manage` route or matching runtime variables.
