...
async function sendQuotePackageEmail(opts: { to: string; cc?: string | null; subject: string; text: string; html?: string | null; }) {
  const apiKey = Deno.env.get('RESEND_API_KEY');
  const from = Deno.env.get('RESEND_FROM_EMAIL') || Deno.env.get('EMAIL_FROM');
  const to = String(opts.to || '').trim();
  if (!apiKey || !from || !to) return { attempted: false, status: 'pending', error: 'Missing email delivery config.' };
  ...
  if (cc) payload.cc = cc.split(/[;,]/).map((v) => v.trim()).filter(Boolean);
  ...
  try { providerMessageId = JSON.parse(body || '{}')?.id || ''; } catch {}
  return { attempted: true, status: 'sent', error: '', provider: 'resend', providerMessageId };
}
...
function evaluateCommercialThresholds(thresholds: any[], context: { totalAmount:number; discountPercent:number; marginPercent:number; jobFamily?:string | null; clientId?:string | null; clientSiteId?:string | null; actorRole?:string | null; jobId?:string | number | null; }) {
  const applicable = (thresholds || []).filter((row) => {
    ...
    if (String(row.entity_type || 'work_order') !== 'work_order') return false;
    const scope = String(row.applies_to_scope || 'global');
    const scopeValue = String(row.applies_to_value || '');
    let scopeMatch = scope === 'global';
    if (scope === 'job_family') scopeMatch = scopeValue === String(context.jobFamily || '');
    else if (scope === 'client') scopeMatch = scopeValue === String(context.clientId || '') || String(row.applies_to_client_id || '') === String(context.clientId || '');
    else if (scope === 'site') scopeMatch = scopeValue === String(context.clientSiteId || '') || String(row.applies_to_client_site_id || '') === String(context.clientSiteId || '');
    else if (scope === 'role') scopeMatch = scopeValue === String(context.actorRole || '') || String(row.applies_to_role || '') === String(context.actorRole || '');
    else if (scope === 'job') scopeMatch = scopeValue === String(context.jobId || '');
    else if (scope === 'service_pattern') scopeMatch = scopeValue === String(context.jobFamily || '');
    if (!scopeMatch) return false;
    if (row.applies_to_client_id && String(row.applies_to_client_id) !== String(context.clientId || '')) return false;
    if (row.applies_to_client_site_id && String(row.applies_to_client_site_id) !== String(context.clientSiteId || '')) return false;
    if (row.applies_to_role && String(row.applies_to_role) !== String(context.actorRole || '')) return false;
    return true;
  });
  let status = 'pass';
  ...
  for (const row of applicable) {
    const marginBreach = row.minimum_margin_percent != null && context.marginPercent < Number(row.minimum_margin_percent || 0);
    const discountBreach = row.discount_percent_cap != null && context.discountPercent > Number(row.discount_percent_cap || 0);
    const totalBreach = row.maximum_total_without_approval != null && context.totalAmount > Number(row.maximum_total_without_approval || 0);
    const block = (marginBreach || discountBreach || totalBreach) && row.hard_block;
    const warn = block || marginBreach || discountBreach || totalBreach;
    if (warn) {
      matchedThreshold = row;
      requiredSignoffRole = row.required_signoff_role || 'admin';
      status = block ? 'block' : 'warn';
      message = block ? (row.block_text || `Blocked by threshold: ${row.threshold_name || 'commercial threshold'}`) : (row.warning_text || `Approval required: ${row.threshold_name || 'commercial threshold'}`);
...
function buildQuotePackageArtifacts(opts: { estimate: any; lines?: any[]; taxProfile?: any | null; brand?: any | null; }) {
  const estimate = opts.estimate || {};
  const lines = Array.isArray(opts.lines) ? opts.lines : [];
  const taxProfile = opts.taxProfile || null;
  const brand = opts.brand || {};
  const brandName = brand.brand_name || taxProfile?.legal_entity_name || taxProfile?.profile_name || 'Your Business';
  const brandTagline = brand.brand_tagline || '';
  const brandEmail = brand.brand_email || '';
  const brandPhone = brand.brand_phone || '';
  const brandAddress = brand.brand_address || '';
  const renderedTitle = brand.rendered_title || estimate.quote_title || estimate.estimate_number || 'Estimate Package';
  const entityLine = taxProfile ? `${taxProfile.legal_entity_name || taxProfile.profile_name || ''}${taxProfile.legal_entity_type ? ` (${taxProfile.legal_entity_type})` : ''}` : '';
  const filingLine = taxProfile ? [taxProfile.federal_return_type, taxProfile.provincial_return_type, taxProfile.usa_tax_classification].filter(Boolean).join(' / ') : '';
  const clientLine = [estimate.client_name, estimate.client_code].filter(Boolean).join(' — ');
  const scopeNotes = estimate.scope_notes || 'Scope details not entered yet.';
  const termsNotes = estimate.terms_notes || 'Standard terms will apply until customized.';
  const clientNotes = estimate.client_notes || '';
  const intro = `Prepared for ${clientLine || 'your client'} on ${new Date().toISOString().slice(0,10)}.`;
  const markdownLines = lines.length ? lines.map((line:any, idx:number) => {
    const qty = Number(line.quantity || 0).toFixed(2);
    const unit = money(line.unit_price || 0);
    const total = money(line.line_total || 0);
    return `${idx + 1}. ${line.description || 'Line item'} — qty ${qty} @ ${unit} = ${total}`;
  }) : ['- No estimate lines have been entered yet.'];
  const previewText = `${renderedTitle} • ${money(estimate.total_amount || 0)} • ${estimate.client_name || 'Client TBD'}`;
  const renderedMarkdown = [
    `# ${renderedTitle}`,'',intro,
    entityLine ? `Entity: ${entityLine}` : '',
    filingLine ? `Entity / filing profile: ${filingLine}` : '',
    '',
    `Estimate #: ${estimate.estimate_number || ''}`,
    `Approval status: ${estimate.approval_status || ''}`,
    `Subtotal: ${money(estimate.subtotal || 0)}`,
    `Tax: ${money(estimate.tax_total || 0)}`,
    `Total: ${money(estimate.total_amount || 0)}`,
    `Estimated margin: ${money(estimate.margin_estimate_total || 0)} (${Number(estimate.margin_estimate_percent || 0).toFixed(2)}%)`,
    '', '## Scope', scopeNotes, '', '## Estimate Lines', ...markdownLines, '', '## Terms', termsNotes, '', '## Client Notes', clientNotes || 'No client notes entered.', '', '## Acceptance',
    'Authorized signature: ______________________________',
    'Printed name: _____________________________________',
    'Date: _____________________________________________',
  ].filter(Boolean).join('\n');
  const lineRows = lines.length ? lines.map((line:any) => `
    <tr>
      <td>${escHtml(line.description || 'Line item')}</td>
      <td style="text-align:right;">${escHtml(Number(line.quantity || 0).toFixed(2))}</td>
      <td style="text-align:right;">${escHtml(money(line.unit_price || 0))}</td>
      <td style="text-align:right;">${escHtml(money(line.line_total || 0))}</td>
    </tr>`).join('') : `
    <tr><td colspan="4" style="color:#6b7280;">No estimate lines have been entered yet.</td></tr>`;
  const printableHtml = `<!doctype html>
<html><head><meta charset="utf-8" /><title>${escHtml(renderedTitle)}</title>
<style>body{font-family:Arial,sans-serif;color:#0f172a;padding:28px;line-height:1.45}.quote-shell{max-width:900px;margin:0 auto}.header{display:flex;justify-content:space-between;gap:24px;border-bottom:2px solid #e2e8f0;padding-bottom:16px;margin-bottom:18px}.brand h1{margin:0 0 4px;font-size:28px}.brand .tag{color:#475569;font-size:14px}.meta{text-align:right;color:#334155}.meta strong{display:block;color:#0f172a}.panel{border:1px solid #dbe4ee;border-radius:12px;padding:14px 16px;margin:14px 0}.panel h2{margin:0 0 8px;font-size:18px}.table{width:100%;border-collapse:collapse;margin-top:8px}.table th,.table td{padding:10px 8px;border-bottom:1px solid #e2e8f0;vertical-align:top}.table th{text-align:left;background:#f8fafc}.totals{margin-top:12px;width:340px;margin-left:auto}.totals td{padding:6px 8px}.accept{margin-top:18px;border-top:1px dashed #94a3b8;padding-top:14px}.small{color:#64748b;font-size:12px}</style></head><body><div class="quote-shell"><div class="header"><div class="brand"><h1>${escHtml(brandName)}</h1>${brandTagline ? `<div class="tag">${escHtml(brandTagline)}</div>` : ''}${brandEmail ? `<div>${escHtml(brandEmail)}</div>` : ''}${brandPhone ? `<div>${escHtml(brandPhone)}</div>` : ''}${brandAddress ? `<div>${escHtml(brandAddress)}</div>` : ''}${entityLine ? `<div class="small">${escHtml(entityLine)}</div>` : ''}${filingLine ? `<div class="small">Filing profile: ${escHtml(filingLine)}</div>` : ''}</div><div class="meta"><strong>${escHtml(renderedTitle)}</strong><div>Estimate: ${escHtml(estimate.estimate_number || '')}</div><div>Client: ${escHtml(clientLine || 'TBD')}</div><div>Status: ${escHtml(estimate.approval_status || 'draft')}</div></div></div><div class="panel"><h2>Scope</h2><div>${escHtml(scopeNotes).replaceAll('\n','<br>')}</div></div><div class="panel"><h2>Estimate Lines</h2><table class="table"><thead><tr><th>Description</th><th style="text-align:right;">Qty</th><th style="text-align:right;">Unit</th><th style="text-align:right;">Line Total</th></tr></thead><tbody>${lineRows}</tbody></table><table class="totals"><tr><td>Subtotal</td><td style="text-align:right;">${escHtml(money(estimate.subtotal || 0))}</td></tr><tr><td>Tax</td><td style="text-align:right;">${escHtml(money(estimate.tax_total || 0))}</td></tr><tr><td><strong>Total</strong></td><td style="text-align:right;"><strong>${escHtml(money(estimate.total_amount || 0))}</strong></td></tr><tr><td>Estimated Margin</td><td style="text-align:right;">${escHtml(money(estimate.margin_estimate_total || 0))} (${escHtml(Number(estimate.margin_estimate_percent || 0).toFixed(2))}%)</td></tr></table></div><div class="panel"><h2>Terms</h2><div>${escHtml(termsNotes).replaceAll('\n','<br>')}</div></div><div class="panel"><h2>Client Notes</h2><div>${escHtml(clientNotes || 'No client notes entered.').replaceAll('\n','<br>')}</div></div><div class="accept"><div>Authorized signature: ____________________________________</div><div>Printed name: ___________________________________________</div><div>Date: _________________________________________________</div></div></div></body></html>`;
  const emailSubject = `Estimate ${estimate.estimate_number || ''} — ${renderedTitle}`;
  const emailBody = ['Hello,','',`Please review the attached estimate package for ${estimate.client_name || 'your project'}.`,`Estimate number: ${estimate.estimate_number || ''}`,`Total: ${money(estimate.total_amount || 0)}`,filingLine ? `Entity / filing profile: ${filingLine}` : '','', 'Regards,', brandName].filter(Boolean).join('\n');
  return { brandName, renderedTitle, renderedMarkdown, printableHtml, emailSubject, emailBody, previewText, renderPayload: { tax_profile_name: taxProfile?.profile_name || null, legal_entity_type: taxProfile?.legal_entity_type || null, legal_entity_name: taxProfile?.legal_entity_name || null, federal_return_type: taxProfile?.federal_return_type || null, provincial_return_type: taxProfile?.provincial_return_type || null, usa_tax_classification: taxProfile?.usa_tax_classification || null, preview_text: previewText } };
}

async function resolveCloseoutAssetLink(supabase: any, sourceTable?: string | null, sourceId?: string | null) {
  const table = String(sourceTable || '').trim();
  const id = String(sourceId || '').trim();
  if (!table || !id) return null;
  try {
    if (table === 'job_comment_attachments') {
      const { data } = await supabase.from('job_comment_attachments').select('*').eq('id', id).maybeSingle();
      if (!data) return null;
      return { asset_kind: data.attachment_kind === 'photo' ? 'job_comment_attachment' : 'file', label: data.file_name || 'Job comment attachment', asset_url: data.preview_url || data.storage_path || null, notes: data.storage_bucket ? `Bucket: ${data.storage_bucket}` : null };
    }
    if (table === 'submission_images') {
      const { data } = await supabase.from('submission_images').select('*').eq('id', id).maybeSingle();
      if (!data) return null;
      return { asset_kind: 'submission_image', label: data.file_name || data.image_type || 'Submission image', asset_url: data.file_path || null, notes: data.caption || null };
    }
    if (table === 'equipment_evidence_assets') {
      const { data } = await supabase.from('equipment_evidence_assets').select('*').eq('id', id).maybeSingle();
      if (!data) return null;
      return { asset_kind: 'equipment_evidence', label: data.file_name || data.caption || 'Equipment evidence', asset_url: data.preview_url || data.storage_path || null, notes: data.caption || null };
    }
  } catch {
    return null;
  }
...
function selectApplicablePostingRule(rules: any[], context: { scopeValue?: string | null; }) {
  const rows = Array.isArray(rules) ? rules.filter((row) => row?.is_active !== false) : [];
  return rows.find((row) => String(row.applies_to_scope || 'global') === 'global')
    || rows.find((row) => String(row.applies_to_value || '') === String(context.scopeValue || ''))
    || null;
}

function buildAccountantHandoffMarkdown(opts: { review: any; job?: any; workOrder?: any; estimate?: any; quotePackage?: any; taxProfile?: any; closeoutItems?: any[]; closeoutAssets?: any[]; signoffSteps?: any[]; invoiceCandidates?: any[]; journalCandidates?: any[]; invoicePostings?: any[]; journalPostings?: any[]; }) {
  const review = opts.review || {};
  const job = opts.job || {};
  const workOrder = opts.workOrder || {};
  const estimate = opts.estimate || {};
  const quotePackage = opts.quotePackage || {};
  const taxProfile = opts.taxProfile || {};
  const closeoutItems = Array.isArray(opts.closeoutItems) ? opts.closeoutItems : [];
  const closeoutAssets = Array.isArray(opts.closeoutAssets) ? opts.closeoutAssets : [];
  const invoiceCandidates = Array.isArray(opts.invoiceCandidates) ? opts.invoiceCandidates : [];
  const journalCandidates = Array.isArray(opts.journalCandidates) ? opts.journalCandidates : [];
  const signoffSteps = Array.isArray(opts.signoffSteps) ? opts.signoffSteps : [];
  const invoicePostings = Array.isArray(opts.invoicePostings) ? opts.invoicePostings : [];
  const journalPostings = Array.isArray(opts.journalPostings) ? opts.journalPostings : [];
  const entityLine = [taxProfile.legal_entity_name || taxProfile.profile_name, taxProfile.legal_entity_type].filter(Boolean).join(' — ');
  const filingLine = [taxProfile.federal_return_type, taxProfile.provincial_return_type, taxProfile.usa_tax_classification].filter(Boolean).join(' / ');
  return [
    '# Accountant Handoff','',
    `Job: ${job.job_code || review.job_code || review.job_id || ''} ${job.job_name || review.job_name || ''}`.trim(),
    workOrder.work_order_number ? `Work Order: ${workOrder.work_order_number}` : '',
    estimate.estimate_number ? `Estimate: ${estimate.estimate_number}` : '',
    quotePackage.rendered_title ? `Quote Package: ${quotePackage.rendered_title}` : '',
    entityLine ? `Entity: ${entityLine}` : '',
    filingLine ? `Filing profile: ${filingLine}` : '',
    '',
    `Review status: ${review.review_status || ''}`,
    `Completion date: ${review.completion_date || ''}`,
    `Revenue: ${money(review.revenue_total || 0)}`,
    `Cost: ${money(review.cost_total || 0)}`,
    `Profit: ${money(review.profit_total || 0)}`,
    `Margin: ${Number(review.margin_percent || 0).toFixed(2)}%`,
    '', '## Variance Summary', review.variance_summary || 'No variance summary entered.','',
    '## Closeout Items', ...(closeoutItems.length ? closeoutItems.map((item:any) => `- ${item.label || item.item_type} [${item.item_status || ''}]${item.notes ? ` — ${item.notes}` : ''}`) : ['- No closeout items']), '',
    '## Closeout Evidence', ...(closeoutAssets.length ? closeoutAssets.map((asset:any) => `- ${asset.label || asset.asset_kind || 'Asset'}${asset.asset_url ? ` — ${asset.asset_url}` : ''}`) : ['- No linked evidence assets']), '',
    '## Invoice Candidates', ...(invoiceCandidates.length ? invoiceCandidates.map((row:any) => `- ${row.candidate_number || row.id} [${row.candidate_status || ''}] ${money(row.total_amount || 0)}`) : ['- No invoice candidates']), '',
    '## Journal Candidates', ...(journalCandidates.length ? journalCandidates.map((row:any) => `- ${row.id} [${row.candidate_status || ''}] ${row.journal_memo || ''}`) : ['- No journal candidates']),
  ].filter(Boolean).join('\n');
}
...
async function recomputeArInvoiceBalance(supabase: any, invoiceId?: string | null) {
  const id = String(invoiceId || '').trim();
  if (!id) return null;
  const invoice:any = await fetchSingle(supabase, 'ar_invoices', id);
  if (!invoice?.id) return null;
  const rows:any[] = await safeSelect(supabase, 'ar_payment_applications', '*', (query:any) => query.eq('invoice_id', id));
  const appliedTotal = rows.filter((row:any) => String(row.application_status || 'applied') !== 'void').reduce((sum:number, row:any) => sum + Number(row.applied_amount || 0), 0);
  const balanceDue = Math.max(Number(invoice.total_amount || 0) - appliedTotal, 0);
  await supabase.from('ar_invoices').update({ balance_due: balanceDue, updated_at: new Date().toISOString() }).eq('id', id);
  return { appliedTotal, balanceDue };
}

async function recomputeArPaymentBalance(supabase: any, paymentId?: string | null) {
  const id = String(paymentId || '').trim();
  if (!id) return null;
  const payment:any = await fetchSingle(supabase, 'ar_payments', id);
  if (!payment?.id) return null;
  const rows:any[] = await safeSelect(supabase, 'ar_payment_applications', '*', (query:any) => query.eq('payment_id', id));
  const appliedTotal = rows.filter((row:any) => String(row.application_status || 'applied') !== 'void').reduce((sum:number, row:any) => sum + Number(row.applied_amount || 0), 0);
  const unappliedAmount = Math.max(Number(payment.amount || 0) - appliedTotal, 0);
  const applicationStatus = unappliedAmount <= 0 ? 'applied' : appliedTotal > 0 ? 'partial' : 'unapplied';
  await supabase.from('ar_payments').update({ unapplied_amount: unappliedAmount, application_status: applicationStatus, last_applied_at: rows.length ? new Date().toISOString() : payment.last_applied_at || null, updated_at: new Date().toISOString() }).eq('id', id);
  return { appliedTotal, unappliedAmount, applicationStatus };
}

async function recomputeApBillBalance(supabase: any, billId?: string | null) {
  const id = String(billId || '').trim();
  if (!id) return null;
  const bill:any = await fetchSingle(supabase, 'ap_bills', id);
  if (!bill?.id) return null;
  const rows:any[] = await safeSelect(supabase, 'ap_payment_applications', '*', (query:any) => query.eq('bill_id', id));
  const appliedTotal = rows.filter((row:any) => String(row.application_status || 'applied') !== 'void').reduce((sum:number, row:any) => sum + Number(row.applied_amount || 0), 0);
  const balanceDue = Math.max(Number(bill.total_amount || 0) - appliedTotal, 0);
  await supabase.from('ap_bills').update({ balance_due: balanceDue, updated_at: new Date().toISOString() }).eq('id', id);
  return { appliedTotal, balanceDue };
}

async function recomputeApPaymentBalance(supabase: any, paymentId?: string | null) {
  const id = String(paymentId || '').trim();
  if (!id) return null;
  const payment:any = await fetchSingle(supabase, 'ap_payments', id);
  if (!payment?.id) return null;
  const rows:any[] = await safeSelect(supabase, 'ap_payment_applications', '*', (query:any) => query.eq('payment_id', id));
  const appliedTotal = rows.filter((row:any) => String(row.application_status || 'applied') !== 'void').reduce((sum:number, row:any) => sum + Number(row.applied_amount || 0), 0);
  const unappliedAmount = Math.max(Number(payment.amount || 0) - appliedTotal, 0);
  const applicationStatus = unappliedAmount <= 0 ? 'applied' : appliedTotal > 0 ? 'partial' : 'unapplied';
  await supabase.from('ap_payments').update({ unapplied_amount: unappliedAmount, application_status: applicationStatus, last_applied_at: rows.length ? new Date().toISOString() : payment.last_applied_at || null, updated_at: new Date().toISOString() }).eq('id', id);
  return { appliedTotal, unappliedAmount, applicationStatus };
}

function buildGeneratedJournalLines(candidate: any, batchId: string, actorId: string) {
  const ls = candidate?.ledger_summary || {};
  const revenueTotal = Number(ls.revenue_total || 0);
  const costTotal = Number(ls.cost_total || 0);
  const revenueAccountId = String(ls.revenue_account_id || '').trim();
  const cogsAccountId = String(ls.cogs_account_id || '').trim();
  const wipAccountId = String(ls.wip_account_id || '').trim() || revenueAccountId || cogsAccountId;
  const entryDate = new Date().toISOString().slice(0, 10);
  const lines:any[] = [];
  const pushLine = (lineNumber:number, accountId:string, debit:number, credit:number, memo:string) => {
    if (!accountId || (!(debit > 0) && !(credit > 0))) return;
    lines.push({ batch_id: batchId, line_number: lineNumber, entry_date: entryDate, account_id: accountId, debit_amount: debit > 0 ? debit : 0, credit_amount: credit > 0 ? credit : 0, source_record_type: 'job_journal_candidate', source_record_id: candidate?.id || null, memo, created_by_profile_id: actorId });
  };
  if (revenueTotal > 0 && wipAccountId) pushLine(10, wipAccountId, revenueTotal, 0, `Generated revenue offset for ${candidate?.job_code || 'job'}`);
  if (revenueTotal > 0 && revenueAccountId) pushLine(20, revenueAccountId, 0, revenueTotal, `Generated revenue recognition for ${candidate?.job_code || 'job'}`);
  if (costTotal > 0 && cogsAccountId) pushLine(30, cogsAccountId, costTotal, 0, `Generated cost recognition for ${candidate?.job_code || 'job'}`);
  if (costTotal > 0 && wipAccountId) pushLine(40, wipAccountId, 0, costTotal, `Generated cost offset for ${candidate?.job_code || 'job'}`);
  return lines;
}

async function syncGlBatchTotals(supabase: any, batchId?: string | null) {
  const id = String(batchId || '').trim();
  if (!id) return null;
  const lines:any[] = await safeSelect(supabase, 'gl_journal_entries', '*', (query:any) => query.eq('batch_id', id));
  const debitTotal = lines.reduce((sum:number, row:any) => sum + Number(row.debit_amount || 0), 0);
  const creditTotal = lines.reduce((sum:number, row:any) => sum + Number(row.credit_amount || 0), 0);
  const lineCount = lines.length;
  const isBalanced = Math.abs(debitTotal - creditTotal) < 0.005;
  await supabase.from('gl_journal_batches').update({ line_count: lineCount, debit_total: debitTotal, credit_total: creditTotal, is_balanced: isBalanced, updated_at: new Date().toISOString() }).eq('id', id);
  return { lineCount, debitTotal, creditTotal, isBalanced };
}
...
if (!isAdmin) return Response.json({ ok: false, error: 'Admin role required' }, { status: 403, headers: corsHeaders });
    ...
    if (entity === 'job_completion_review') {
      ...
      if (action === 'create_invoice_candidate') {
        ...
        const rules:any[] = await safeSelect(supabase, 'invoice_candidate_posting_rules', '*', (query:any) => query.eq('is_active', true));
        const postingRule:any = selectApplicablePostingRule(rules, { scopeValue: estimate?.job_family || null });
        const closeoutItems:any[] = await safeSelect(supabase, 'job_completion_closeout_items', '*', (query:any) => query.eq('completion_review_id', review.id));
        const closeoutComplete = closeoutItems.filter((row:any) => row.required_item !== false).every((row:any) => String(row.item_status || '') === 'complete');
        if (postingRule?.require_approved_completion && !['approved','ready_for_accounting'].includes(String(review.review_status || ''))) return Response.json({ ok:false, error:'Invoice posting rule requires an approved completion review.' }, { status:400, headers:corsHeaders });
        if (postingRule?.require_released_work_order && workOrder?.status !== 'released') return Response.json({ ok:false, error:'Invoice posting rule requires a released work order.' }, { status:400, headers:corsHeaders });
        if (postingRule?.require_closeout_complete && !closeoutComplete) return Response.json({ ok:false, error:'Invoice posting rule requires all required closeout items to be complete.' }, { status:400, headers:corsHeaders });
        const candidateNumber = body.candidate_number || `INV-CAND-${String(review.job_id).padStart(4,'0')}`;
        const subtotal = asNumber(body.subtotal, review.revenue_total || 0);
        const taxTotal = asNumber(body.tax_total, 0);
        const totalAmount = asNumber(body.total_amount, subtotal + taxTotal);
        const { data, error } = await supabase.from('job_invoice_candidates').insert({ completion_review_id: review.id, job_id: review.job_id, work_order_id: review.work_order_id || null, estimate_id: review.estimate_id || null, business_tax_setting_id: asNullableText(body.business_tax_setting_id), candidate_status: body.candidate_status || 'ready', candidate_number: candidateNumber, client_id: workOrder?.client_id || estimate?.client_id || null, client_site_id: workOrder?.client_site_id || estimate?.client_site_id || null, subtotal, tax_total: taxTotal, total_amount: totalAmount, memo: body.memo ?? 'Generated from completion review.', payload: { review_status: review.review_status, margin_percent: review.margin_percent || 0, posting_rule_id: postingRule?.id || null, ar_account_id: postingRule?.ar_account_id || null, offset_account_id: postingRule?.offset_account_id || null, tax_liability_account_id: postingRule?.tax_liability_account_id || null }, created_by_profile_id: actorId }).select('*').single();
        if (error) throw error;
        await supabase.from('job_completion_accounting_events').insert({ completion_review_id: review.id, job_id: review.job_id, accounting_action: 'create_invoice_candidate', event_status: 'completed', memo: body.memo ?? 'Invoice candidate created.', payload: { candidate_id: data.id, candidate_number: candidateNumber, posting_rule_id: postingRule?.id || null }, created_by_profile_id: actorId, completed_at: new Date().toISOString() });
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'create_journal_candidate') {
        const review:any = await fetchSingle(supabase, 'job_completion_reviews', body.item_id);
        if (!review?.id) return Response.json({ ok:false, error:'Completion review not found.' }, { status:404, headers:corsHeaders });
        const rules:any[] = await safeSelect(supabase, 'journal_candidate_posting_rules', '*', (query:any) => query.eq('is_active', true));
        const postingRule:any = selectApplicablePostingRule(rules, { scopeValue: null });
        const closeoutItems:any[] = await safeSelect(supabase, 'job_completion_closeout_items', '*', (query:any) => query.eq('completion_review_id', review.id));
        const closeoutComplete = closeoutItems.filter((row:any) => row.required_item !== false).every((row:any) => String(row.item_status || '') === 'complete');
        if (postingRule?.require_approved_completion && !['approved','ready_for_accounting'].includes(String(review.review_status || ''))) return Response.json({ ok:false, error:'Journal posting rule requires an approved completion review.' }, { status:400, headers:corsHeaders });
        if (postingRule?.require_closeout_complete && !closeoutComplete) return Response.json({ ok:false, error:'Journal posting rule requires all required closeout items to be complete.' }, { status:400, headers:corsHeaders });
        const ledgerSummary = body.ledger_summary || { revenue_total: review.revenue_total || 0, cost_total: review.cost_total || 0, profit_total: review.profit_total || 0, revenue_account_id: postingRule?.revenue_account_id || null, cogs_account_id: postingRule?.cogs_account_id || null, wip_account_id: postingRule?.wip_account_id || null, variance_account_id: postingRule?.variance_account_id || null };
        const { data, error } = await supabase.from('job_journal_candidates').insert({ completion_review_id: review.id, job_id: review.job_id, business_tax_setting_id: asNullableText(body.business_tax_setting_id), candidate_status: body.candidate_status || 'ready', journal_memo: body.journal_memo ?? `Closeout journal candidate for job ${review.job_id}`, ledger_summary: ledgerSummary, payload: { review_status: review.review_status, accounting_ready: review.accounting_ready === true, posting_rule_id: postingRule?.id || null }, created_by_profile_id: actorId }).select('*').single();
        if (error) throw error;
        await supabase.from('job_completion_accounting_events').insert({ completion_review_id: review.id, job_id: review.job_id, accounting_action: 'create_journal_candidate', event_status: 'completed', memo: body.journal_memo ?? 'Journal candidate created.', payload: { candidate_id: data.id, posting_rule_id: postingRule?.id || null }, created_by_profile_id: actorId, completed_at: new Date().toISOString() });
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      ...
      if (action === 'export_accountant_handoff') {
        ...
        if (!review?.id) return Response.json({ ok:false, error:'Completion review not found.' }, { status:404, headers:corsHeaders });
        const job:any = review.job_id ? await fetchSingle(supabase, 'jobs', review.job_id) : null;
        const workOrder:any = review.work_order_id ? await fetchSingle(supabase, 'work_orders', review.work_order_id) : null;
        const estimate:any = review.estimate_id ? await fetchSingle(supabase, 'estimates', review.estimate_id) : null;
        const quotePkg = review.estimate_id ? await supabase.from('estimate_quote_packages').select('*').eq('estimate_id', review.estimate_id).maybeSingle() : { data: null };
        const taxProfile = asNullableText(body.business_tax_setting_id) ? await supabase.from('business_tax_settings').select('*').eq('id', asNullableText(body.business_tax_setting_id)).maybeSingle() : { data: null };
        const { data: closeoutItems } = await supabase.from('v_job_completion_package_directory').select('*').eq('completion_review_id', review.id).order('sort_order');
        const { data: closeoutAssets } = await supabase.from('v_job_closeout_evidence_directory').select('*').eq('completion_review_id', review.id).order('created_at');
        const { data: signoffSteps } = await supabase.from('v_job_completion_signoff_directory').select('*').eq('completion_review_id', review.id).order('sort_order');
        const { data: invoiceCandidates } = await supabase.from('v_job_invoice_candidate_directory').select('*').eq('completion_review_id', review.id).order('created_at');
        const { data: journalCandidates } = await supabase.from('v_job_journal_candidate_directory').select('*').eq('completion_review_id', review.id).order('created_at');
        const { data: invoicePostings } = await supabase.from('v_job_invoice_posting_directory').select('*').eq('completion_review_id', review.id).order('created_at');
        const { data: journalPostings } = await supabase.from('v_job_journal_posting_directory').select('*').eq('completion_review_id', review.id).order('created_at');
        const payload = { job_code: review.job_code, review_status: review.review_status, revenue_total: review.revenue_total, cost_total: review.cost_total, profit_total: review.profit_total, accounting_ready: review.accounting_ready, estimate_id: review.estimate_id, work_order_id: review.work_order_id, quote_package: quotePkg.data || null, closeout_item_count: (closeoutItems || []).length, closeout_asset_count: (closeoutAssets || []).length, signoff_step_count: (signoffSteps || []).length, invoice_candidate_count: (invoiceCandidates || []).length, journal_candidate_count: (journalCandidates || []).length, invoice_posting_count: (invoicePostings || []).length, journal_posting_count: (journalPostings || []).length, tax_profile: taxProfile.data || null };
        const md = buildAccountantHandoffMarkdown({ review, job, workOrder, estimate, quotePackage: quotePkg.data || null, taxProfile: taxProfile.data || null, closeoutItems: closeoutItems || [], closeoutAssets: closeoutAssets || [], signoffSteps: signoffSteps || [], invoiceCandidates: invoiceCandidates || [], journalCandidates: journalCandidates || [], invoicePostings: invoicePostings || [], journalPostings: journalPostings || [] });
        const titlePrefix = taxProfile.data?.federal_return_type === 'T2' ? 'Corporation / T2 accountant handoff' : ((taxProfile.data?.legal_entity_type || '').includes('llc') ? 'LLC bookkeeping handoff' : 'Accountant handoff');
        const { data, error } = await supabase.from('accountant_handoff_exports').insert({ export_kind: body.export_kind || (taxProfile.data?.federal_return_type === 'T2' ? 't2_package' : ((taxProfile.data?.legal_entity_type || '').includes('llc') ? 'llc_package' : 'closeout_bundle')), entity_scope: 'completion_review', entity_id: String(review.id), business_tax_setting_id: asNullableText(body.business_tax_setting_id), export_status: 'generated', export_title: `${titlePrefix} - ${review.job_code || review.job_id}`, export_markdown: md, export_payload: payload, generated_by_profile_id: actorId, generated_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select('*').single();
        if (error) throw error;
    ...
    if (entity === 'estimate_quote_package') {
      ...
      if (action === 'render_from_estimate') {
        ...
        const artifacts = buildQuotePackageArtifacts({ estimate, lines: lines || [], taxProfile, brand: body || {} });
        const upsert = { ...patch, business_tax_setting_id: taxProfileId, package_status: body.package_status || 'rendered', rendered_title: artifacts.renderedTitle, rendered_markdown: artifacts.renderedMarkdown, rendered_html: artifacts.printableHtml, printable_html: artifacts.printableHtml, email_subject: artifacts.emailSubject, email_body: artifacts.emailBody, brand_name: artifacts.brandName, brand_tagline: body.brand_tagline ?? null, brand_email: body.brand_email ?? null, brand_phone: body.brand_phone ?? null, brand_address: body.brand_address ?? null, last_rendered_at: new Date().toISOString(), render_payload: { ...(artifacts.renderPayload || {}), estimate_number: estimate.estimate_number || null, client_name: estimate.client_name || null, total_amount: estimate.total_amount || 0 } };
      ...
      if (action === 'send_quote_output') {
        ...
        const email = asNullableText(body.client_email) || asNullableText(body.recipient_email) || asNullableText(quotePkg.client_email);
        if (!email) return Response.json({ ok:false, error:'Recipient email is required.' }, { status:400, headers:corsHeaders });
        const sendPatch:any = {
          client_email: email,
          recipient_name: body.recipient_name ?? quotePkg.recipient_name ?? null,
          recipient_profile_id: asNullableText(body.recipient_profile_id) || quotePkg.recipient_profile_id || null,
          copied_to_emails: body.copied_to_emails ?? quotePkg.copied_to_emails ?? null,
          last_sent_via: body.output_type || 'email',
          last_send_attempt_at: new Date().toISOString(),
          send_status: 'sending',
          updated_at: new Date().toISOString()
        };
        await supabase.from('estimate_quote_packages').update(sendPatch).eq('id', quotePkg.id);
        try {
          const subject = String(body.email_subject || quotePkg.email_subject || quotePkg.rendered_title || 'Quote package');
          const text = String(body.email_body || quotePkg.email_body || quotePkg.rendered_markdown || 'Please review your quote package.');
          ...
          await supabase.from('estimate_quote_packages').update({ send_status: 'failed', send_error: message, updated_at: new Date().toISOString() }).eq('id', quotePkg.id);
          await supabase.from('quote_package_output_events').insert({ quote_package_id: body.item_id, output_type: body.output_type || 'email', output_status: 'failed', recipient_email: email, email_subject: body.email_subject || quotePkg.email_subject || null, provider_name: 'resend', output_error: message, output_payload: { rendered_title: quotePkg.rendered_title || null }, created_by_profile_id: actorId });
    ...
    if (entity === 'job_completion_closeout_asset') {
      const resolved = await resolveCloseoutAssetLink(supabase, body.source_table, body.source_id);
      const patch = {
        closeout_item_id: asNullableText(body.closeout_item_id),
        asset_kind: body.asset_kind ?? resolved?.asset_kind ?? 'url',
        label: body.label ?? resolved?.label ?? null,
        asset_url: body.asset_url ?? resolved?.asset_url ?? null,
        source_table: body.source_table ?? null,
        source_id: body.source_id ?? null,
        notes: body.notes ?? resolved?.notes ?? null,
        created_by_profile_id: actorId,
        updated_at: new Date().toISOString(),
      };
    ...
    if (entity === 'accountant_handoff_export') {
      ...
      if (action === 'build_bundle') {
        ...
        const arApps = await safeList(supabase, 'v_ar_payment_application_directory', '*', 'application_date', 120, true);
        const apApps = await safeList(supabase, 'v_ap_payment_application_directory', '*', 'application_date', 120, true);
        const taxPrep = await safeList(supabase, 'v_sales_tax_prep_directory', '*', 'period_end', 50, false);
        const taxReview = await safeList(supabase, 'v_sales_tax_filing_review_directory', '*', 'filing_period_end', 50, false);
        const payrollPrep = await safeList(supabase, 'v_payroll_remittance_prep_directory', '*', 'period_end', 50, false);
        const payrollReview = await safeList(supabase, 'v_payroll_remittance_review_directory', '*', 'remittance_period_end', 50, false);
        const scoredMatches = await safeList(supabase, 'v_bank_reconciliation_match_scored_directory', '*', 'match_score', 120, false);
        const packageRows = [
          ...
          { kind: 'ar_payment_applications', label: 'AR payment applications', payload: { row_count: arApps.length, rows: arApps.slice(0, 120) } },
          { kind: 'ap_payment_applications', label: 'AP payment applications', payload: { row_count: apApps.length, rows: apApps.slice(0, 120) } },
          { kind: 'sales_tax_prep', label: 'Sales tax prep', payload: { row_count: taxPrep.length, rows: taxPrep.slice(0, 24) } },
          { kind: 'sales_tax_review', label: 'Sales tax filing review', payload: { row_count: taxReview.length, rows: taxReview.slice(0, 24) } },
          ...
          { kind: 'payroll_remittance_review', label: 'Payroll remittance review', payload: { row_count: payrollReview.length, rows: payrollReview.slice(0, 24) } },
          { kind: 'reconciliation_matches', label: 'Bank reconciliation match candidates', payload: { row_count: scoredMatches.length, rows: scoredMatches.slice(0, 80) } },
        ];
        for (let i = 0; i < packageRows.length; i += 1) {
        ...
        const bundlePayload = { section_count: sections.length, built_at: new Date().toISOString(), kinds: packageRows.map((row) => row.kind) };
        const packageManifest = { generated_at: new Date().toISOString(), section_count: sections.length, item_kinds: packageRows.map((row) => ({ kind: row.kind, label: row.label, row_count: row.payload.row_count })) };
        const md = ['# Accountant Handoff Package', '', `Sections: ${sections.length}`, '', ...packageRows.map((row) => `- ${row.label}: ${row.payload.row_count} row(s)`)].join('\n');
        const { data, error } = await supabase.from('accountant_handoff_exports').update({ export_status: 'generated', package_status: 'prepared', bundle_item_count: sections.length, bundle_payload: bundlePayload, package_manifest: packageManifest, package_markdown: md, package_json: { sections: packageRows }, export_markdown: md, generated_by_profile_id: actorId, generated_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', exportId).select('*').single();
        if (error) throw error;
        await recordJobAccountingLifecycleEvent(supabase, { job_id: null, source_type: 'accountant_export', source_id: exportId, lifecycle_stage: 'accountant_package_prepared', lifecycle_status: 'prepared', headline: 'Accountant handoff package prepared', notes: `${sections.length} package sections prepared.`, created_by_profile_id: actorId });
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
    ...
    if (entity === 'job_completion_signoff_step') {
      const patch = {
        completion_review_id: asNullableText(body.completion_review_id),
        signoff_kind: body.signoff_kind ?? 'field_supervisor',
        signoff_status: body.signoff_status ?? 'pending',
        required_role: body.required_role ?? null,
        signoff_profile_id: asNullableText(body.signoff_profile_id),
        signoff_name: body.signoff_name ?? null,
        signoff_notes: body.signoff_notes ?? null,
        signed_at: asNullableDateTime(body.signed_at),
        sort_order: asNumber(body.sort_order, 100),
        created_by_profile_id: actorId,
        updated_at: new Date().toISOString(),
      };
      if (!patch.completion_review_id) return Response.json({ ok:false, error:'completion_review_id is required' }, { status:400, headers:corsHeaders });
      if (action === 'create') {
        const { data, error } = await supabase.from('job_completion_signoff_steps').insert(patch).select('*').single();
        if (error) throw error;
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'update') {
        const { data, error } = await supabase.from('job_completion_signoff_steps').update(patch).eq('id', body.item_id).select('*').single();
        if (error) throw error;
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'sign') {
      ...
      if (action === 'delete') {
        const { error } = await supabase.from('job_completion_signoff_steps').delete().eq('id', body.item_id);
        if (error) throw error;
        return Response.json({ ok:true }, { headers:corsHeaders });
      }
    }

    if (entity === 'job_invoice_candidate') {
      if (action === 'post_candidate') {
    ...
    if (entity === 'job_journal_candidate') {
      if (action === 'post_candidate') {
        ...
        let generatedLines:any[] = [];
        const existingLines:any[] = glBatch?.id ? await safeSelect(supabase, 'gl_journal_entries', '*', (query:any) => query.eq('batch_id', glBatch.id)) : [];
        if (!existingLines.length && glBatch?.id) {
          generatedLines = buildGeneratedJournalLines(candidate, glBatch.id, actorId);
          if (generatedLines.length) {
            const { error: lineError } = await supabase.from('gl_journal_entries').insert(generatedLines);
            if (lineError) throw lineError;
          }
        }
        const batchSummary = await syncGlBatchTotals(supabase, glBatch?.id || null);
        const payload = { ...(candidate.payload || {}), ledger_summary: candidate.ledger_summary || {}, ar_ap_queue_id: queue.data?.id || null, external_system: body.external_system || 'manual', journal_entry_number: body.journal_entry_number || null, batch_number: glBatch?.batch_number || batchNumber, gl_batch_id: glBatch?.id || null, generated_line_count: generatedLines.length || existingLines.length, batch_summary: batchSummary || null };
        ...
        return Response.json({ ok:true, record:data, gl_batch: glBatch, generated_lines: generatedLines.length || existingLines.length, batch_summary: batchSummary }, { headers:corsHeaders });
      }
    }
    ...
    if (entity === 'sales_tax_filing') {
      const patch = {
        ...
        paid_at: asNullableDateTime(body.paid_at),
        created_by_profile_id: actorId,
        updated_at: new Date().toISOString(),
      };
    ...
    if (entity === 'payroll_remittance_run') {
      ...
      if (action === 'prepare_from_export') {
        ...
        if (exportRun) {
          ...
          patch.review_status = 'prepared';
        }
      }
      if (action === 'update' || action === 'review' || action === 'approve' || action === 'remit' || action === 'prepare_from_export') {
        if (action === 'review') { patch.review_status = 'reviewed'; patch.reviewed_by_profile_id = actorId; patch.reviewed_at = new Date().toISOString(); }
        if (action === 'approve') { patch.review_status = 'approved'; patch.reviewed_by_profile_id = actorId; patch.reviewed_at = new Date().toISOString(); }
        if (action === 'remit') { patch.remittance_status = 'remitted'; patch.review_status = 'remitted'; patch.remitted_by_profile_id = actorId; patch.remitted_at = new Date().toISOString(); }
    ...
    if (entity === 'bank_reconciliation_session') {
      ...
      if (action === 'auto_match') {
        ...
        const candidates:any[] = await safeList(supabase, 'v_bank_reconciliation_match_scored_directory', '*', 'match_score', 800, false);
        const matches = candidates
          .filter((row:any) => String(row.reconciliation_session_id || '') === String(sessionId) && String(row.recommendation || '') === 'auto_match')
        ...
        for (const match of matches) {
          ...
          await supabase.from('bank_reconciliation_items').update({ match_status: 'matched', clearing_status: 'cleared', notes: `Auto-matched score ${match.match_score} with ${match.candidate_source_type} ${match.candidate_source_id}.`, updated_at: new Date().toISOString() }).eq('id', match.bank_item_id);
          await supabase.from('bank_reconciliation_items').update({ match_status: 'matched', clearing_status: 'cleared', notes: `Auto-matched score ${match.match_score} with ${match.bank_item_source_type} ${match.bank_item_source_id}.`, updated_at: new Date().toISOString() }).eq('id', match.candidate_item_id);
          touched.add(String(match.bank_item_id));
          touched.add(String(match.candidate_item_id));
        }
        patch.notes = [patch.notes || null, `${touched.size / 2} scored match(es) auto-applied.`].filter(Boolean).join(' ');
      }
    ...
    if (entity === 'ar_payment') {
      const patch = {
        ...
        reference_number: body.reference_number ?? null,
        amount: asNumber(body.amount, 0),
        unapplied_amount: asNumber(body.unapplied_amount, asNumber(body.amount, 0)),
        application_status: body.application_status ?? 'unapplied',
        last_applied_at: asNullableDateTime(body.last_applied_at),
        last_application_notes: body.last_application_notes ?? null,
        notes: body.notes ?? null,
        created_by_profile_id: actorId,
        updated_at: new Date().toISOString(),
      };
    ...
    if (entity === 'ar_payment_application') {
      ...
      if (action === 'auto_apply') {
        const payment:any = body.payment_id ? await fetchSingle(supabase, 'ar_payments', body.payment_id) : body.item_id ? await fetchSingle(supabase, 'ar_payments', body.item_id) : null;
        if (!payment?.id) return Response.json({ ok:false, error:'AR payment not found.' }, { status:404, headers:corsHeaders });
        const invoices:any[] = await safeSelect(supabase, 'ar_invoices', '*', (query:any) => query.eq('client_id', payment.client_id).gt('balance_due', 0).order('due_date', { ascending:true }).limit(200));
        let remaining = Number(payment.unapplied_amount ?? payment.amount ?? 0);
        const created:any[] = [];
        for (const invoice of invoices) {
          if (remaining <= 0) break;
          const applyAmount = Math.min(remaining, Number(invoice.balance_due || 0));
          if (!(applyAmount > 0)) continue;
          const { data, error } = await supabase.from('ar_payment_applications').insert({ payment_id: payment.id, invoice_id: invoice.id, applied_amount: applyAmount, application_date: new Date().toISOString().slice(0,10), application_status: 'applied', notes: body.notes ?? 'Auto-applied to oldest open invoices.', created_by_profile_id: actorId, updated_at: new Date().toISOString() }).select('*').single();
          if (error) throw error;
          created.push(data);
          remaining -= applyAmount;
          await recomputeArInvoiceBalance(supabase, invoice.id);
        }
        await recomputeArPaymentBalance(supabase, payment.id);
        return Response.json({ ok:true, applied_count: created.length, records: created }, { headers:corsHeaders });
      }
      if (!patch.payment_id || !patch.invoice_id || !(patch.applied_amount > 0)) return Response.json({ ok:false, error:'payment_id, invoice_id, and a positive applied_amount are required' }, { status:400, headers:corsHeaders });
      if (action === 'create') {
        const payment:any = await fetchSingle(supabase, 'ar_payments', patch.payment_id);
        const available = Number(payment?.unapplied_amount ?? payment?.amount ?? 0);
        if (patch.applied_amount > available + 0.005) return Response.json({ ok:false, error:'Applied amount exceeds unapplied payment amount.' }, { status:400, headers:corsHeaders });
        const { data, error } = await supabase.from('ar_payment_applications').insert(patch).select('*').single();
        if (error) throw error;
        await recomputeArInvoiceBalance(supabase, patch.invoice_id);
        await recomputeArPaymentBalance(supabase, patch.payment_id);
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'update') {
        const existing:any = await fetchSingle(supabase, 'ar_payment_applications', body.item_id);
        const { data, error } = await supabase.from('ar_payment_applications').update(patch).eq('id', body.item_id).select('*').single();
        if (error) throw error;
        await recomputeArInvoiceBalance(supabase, patch.invoice_id || existing?.invoice_id);
        await recomputeArInvoiceBalance(supabase, existing?.invoice_id);
        await recomputeArPaymentBalance(supabase, patch.payment_id || existing?.payment_id);
        await recomputeArPaymentBalance(supabase, existing?.payment_id);
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'delete') {
        const existing:any = await fetchSingle(supabase, 'ar_payment_applications', body.item_id);
        const { error } = await supabase.from('ar_payment_applications').delete().eq('id', body.item_id);
        if (error) throw error;
        await recomputeArInvoiceBalance(supabase, existing?.invoice_id);
        await recomputeArPaymentBalance(supabase, existing?.payment_id);
        return Response.json({ ok:true }, { headers:corsHeaders });
    ...
    if (entity === 'ap_payment_application') {
      ...
      if (action === 'auto_apply') {
        const payment:any = body.payment_id ? await fetchSingle(supabase, 'ap_payments', body.payment_id) : body.item_id ? await fetchSingle(supabase, 'ap_payments', body.item_id) : null;
        if (!payment?.id) return Response.json({ ok:false, error:'AP payment not found.' }, { status:404, headers:corsHeaders });
        const bills:any[] = await safeSelect(supabase, 'ap_bills', '*', (query:any) => query.eq('vendor_id', payment.vendor_id).gt('balance_due', 0).order('due_date', { ascending:true }).limit(200));
        let remaining = Number(payment.unapplied_amount ?? payment.amount ?? 0);
        const created:any[] = [];
        for (const bill of bills) {
          if (remaining <= 0) break;
          const applyAmount = Math.min(remaining, Number(bill.balance_due || 0));
          if (!(applyAmount > 0)) continue;
          const { data, error } = await supabase.from('ap_payment_applications').insert({ payment_id: payment.id, bill_id: bill.id, applied_amount: applyAmount, application_date: new Date().toISOString().slice(0,10), application_status: 'applied', notes: body.notes ?? 'Auto-applied to oldest open bills.', created_by_profile_id: actorId, updated_at: new Date().toISOString() }).select('*').single();
          if (error) throw error;
          created.push(data);
          remaining -= applyAmount;
          await recomputeApBillBalance(supabase, bill.id);
        }
        await recomputeApPaymentBalance(supabase, payment.id);
        return Response.json({ ok:true, applied_count: created.length, records: created }, { headers:corsHeaders });
      }
      if (!patch.payment_id || !patch.bill_id || !(patch.applied_amount > 0)) return Response.json({ ok:false, error:'payment_id, bill_id, and a positive applied_amount are required' }, { status:400, headers:corsHeaders });
      if (action === 'create') {
        const payment:any = await fetchSingle(supabase, 'ap_payments', patch.payment_id);
        const available = Number(payment?.unapplied_amount ?? payment?.amount ?? 0);
        if (patch.applied_amount > available + 0.005) return Response.json({ ok:false, error:'Applied amount exceeds unapplied payment amount.' }, { status:400, headers:corsHeaders });
        const { data, error } = await supabase.from('ap_payment_applications').insert(patch).select('*').single();
        if (error) throw error;
        await recomputeApBillBalance(supabase, patch.bill_id);
        await recomputeApPaymentBalance(supabase, patch.payment_id);
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'update') {
        const existing:any = await fetchSingle(supabase, 'ap_payment_applications', body.item_id);
        const { data, error } = await supabase.from('ap_payment_applications').update(patch).eq('id', body.item_id).select('*').single();
        if (error) throw error;
        await recomputeApBillBalance(supabase, patch.bill_id || existing?.bill_id);
        await recomputeApBillBalance(supabase, existing?.bill_id);
        await recomputeApPaymentBalance(supabase, patch.payment_id || existing?.payment_id);
        await recomputeApPaymentBalance(supabase, existing?.payment_id);
        return Response.json({ ok:true, record:data }, { headers:corsHeaders });
      }
      if (action === 'delete') {
        const existing:any = await fetchSingle(supabase, 'ap_payment_applications', body.item_id);
        const { error } = await supabase.from('ap_payment_applications').delete().eq('id', body.item_id);
        if (error) throw error;
        await recomputeApBillBalance(supabase, existing?.bill_id);
        await recomputeApPaymentBalance(supabase, existing?.payment_id);
        return Response.json({ ok:true }, { headers:corsHeaders });
