...
serve(async (req) => {
  ...
  try {
    ...
    if (body.entity === 'equipment' && body.action === 'checkout') {
      ...
      const { data, error } = await supabase.from('equipment_signouts').insert({ equipment_item_id: equipmentId, job_id: jobId, checked_out_by_profile_id: actorProfile.id, checked_out_to_supervisor_profile_id: await resolveProfileIdByNameOrEmail(supabase, body.supervisor_name), signout_notes: body.notes ?? null, checkout_worker_signature_name: body.worker_signature_name ?? null, checkout_supervisor_signature_name: body.supervisor_signature_name ?? null, checkout_admin_signature_name: body.admin_signature_name ?? null, checkout_worker_signature_png: body.worker_signature_png ?? null, checkout_supervisor_signature_png: body.supervisor_signature_png ?? null, checkout_admin_signature_png: body.admin_signature_png ?? null, checkout_condition: body.checkout_condition ?? null, checkout_photos_json: body.checkout_photos_json ? JSON.parse(body.checkout_photos_json) : [] }).select('*').single();
      if (error) throw error;
    ...
    if (body.entity === 'equipment' && body.action === 'return') {
      ...
      if (signout?.id) await supabase.from('equipment_signouts').update({ returned_at:new Date().toISOString(), return_worker_signature_name: body.worker_signature_name ?? null, return_supervisor_signature_name: body.supervisor_signature_name ?? null, return_admin_signature_name: body.admin_signature_name ?? null, return_worker_signature_png: body.worker_signature_png ?? null, return_supervisor_signature_png: body.supervisor_signature_png ?? null, return_admin_signature_png: body.admin_signature_png ?? null, return_condition: body.return_condition ?? null, return_notes: body.return_notes ?? null, return_photos_json: body.return_photos_json ? JSON.parse(body.return_photos_json) : [], damage_reported: !!body.damage_reported, damage_notes: body.damage_notes ?? null }).eq('id', signout.id);
      await insertNotification(supabase, { notification_type:'equipment_return', target_table:'equipment_items', target_id:equipmentId, recipient_role:'admin', title:`Equipment returned: ${body.equipment_code}`, body: JSON.stringify({ equipment_code: body.equipment_code }), created_by_profile_id: actorProfile.id, email_subject: `YWI HSE equipment return: ${body.equipment_code}`, payload: { equipment_code: body.equipment_code, damage_reported: !!body.damage_reported, damage_notes: body.damage_notes ?? null } });
      return Response.json({ ok:true }, { headers:corsHeaders });
    }
