...
serve(async (req) => {
  ...
  try {
    ...
    if (body.entity === 'equipment' && body.action === 'checkout') {
      ...
      const { data, error } = await supabase.from('equipment_signouts').insert({ equipment_item_id: equipmentId, job_id: jobId, checked_out_by_profile_id: actorProfile.id, checked_out_to_supervisor_profile_id: await resolveProfileIdByNameOrEmail(supabase, body.supervisor_name), signout_notes: body.notes ?? null, checkout_worker_signature_name: body.worker_signature_name ?? null, checkout_supervisor_signature_name: body.supervisor_signature_name ?? null, checkout_admin_signature_name: body.admin_signature_name ?? null, checkout_worker_signature_png: body.worker_signature_png ?? null, checkout_supervisor_signature_png: body.supervisor_signature_png ?? null, checkout_admin_signature_png: body.admin_signature_png ?? null, checkout_condition: body.checkout_condition ?? null }).select('*').single();
      if (error) throw error;
