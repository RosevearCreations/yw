...
async function statusPayload(supabase: any) {
  const [{ data: rows, error: rowsError }, { data: assertions, error: assertionError }, { data: recentRuns, error: runsError }] = await Promise.all([
  ...
  const failedAssertions = assertionRows.filter((row:any) => String(row?.assertion_status || '').toLowerCase() !== 'passed');
  ...
  return {
    ...
    security_assertions:assertionRows,
