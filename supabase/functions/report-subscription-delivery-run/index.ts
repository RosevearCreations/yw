...
function makeRunCode(settingCode: string, subscriptionName: string) {
  ...
  const safe = String(subscriptionName || 'subscription')
    .replace(/[^A-Z0-9]+/gi, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 24)
    .toUpperCase() || 'SUB'
...
async function sendEmail(
  ...
  if (!apiKey || !from || !to.length) {
    return {
      attempted: false,
      status: 'pending',
      error: 'Missing email delivery config.',
    }
  }
...
function applyFilterPayload(
  rows: Record<string, unknown>[],
  filterPayload: Record<string, unknown> | null | undefined,
) {
...
async function fetchRowsForKind(supabase: any, sub: SubscriptionRow) {
  ...
  if (kind === 'overdue_corrective_actions') {
    const { data } = await supabase
      .from('v_corrective_action_task_directory')
      .select('*')
      .or('is_overdue.eq.true,reminder_due.eq.true,escalation_due.eq.true')
  ...
  if (kind === 'training_expiry_30_days') {
    const { data } = await supabase
      .from('v_training_record_directory')
      .select('*')
      .or('is_expired.eq.true,expires_within_30_days.eq.true,verification_pending.eq.true')
  ...
  if (kind === 'rejected_evidence_followup') {
    const att = await supabase
      .from('v_attendance_photo_review')
      .select('*')
      .in('review_status', ['rejected', 'follow_up'])
      .order('uploaded_at', { ascending: false })
      .limit(limit)

    const hse = await supabase
      .from('v_hse_evidence_review')
      .select('*')
      .in('review_status', ['rejected', 'follow_up'])
      .order('created_at', { ascending: false })
      .limit(limit)

    return applyFilterPayload(
      ([...(att.data || []), ...(hse.data || [])] as Record<string, unknown>[]),
      sub.filter_payload,
    )
  }
  ...
  if (kind === 'incident_near_miss_summary') {
    ...
    const { data } = await supabase
      .from('v_incident_near_miss_history')
      .select('*')
      .gte('submission_date', cutoff.toISOString().slice(0, 10))
  ...
  const queue = await supabase
    .from('v_supervisor_safety_queue')
    .select('*')
...
function buildSummaryText(sub: SubscriptionRow, rows: Record<string, unknown>[]) {
  ...
  if (kind === 'weekly_supervisor_summary') {
    lines.push(
      `Corrective items: ${
        rows.filter((row) => String(row.queue_type || row.alert_type || '').includes('corrective')).length
      }`,
      `Training items: ${
        rows.filter((row) => String(row.queue_type || row.alert_type || '').includes('training')).length
      }`,
      `SDS items: ${
        rows.filter((row) => String(row.queue_type || row.alert_type || '').includes('sds')).length
      }`,
    )
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const expectedSecret =
      Deno.env.get('REPORT_DELIVERY_SCHEDULER_SECRET') ||
      Deno.env.get('SERVICE_EXECUTION_SCHEDULER_SECRET')
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      return json({ ok: false, error: 'Missing service role configuration' }, 500)
    }
    ...
    const { data: setting, error: settingError } = await supabase
      .from('report_delivery_scheduler_settings')
      .select('*')
    ...
    if (settingError) {
      return json({
        ok: false,
        error: 'Could not read report delivery scheduler setting',
    ...
    if (!setting) {
      return json({
        ok: false,
        error: `No report delivery scheduler setting found for ${settingCode}`,
      }, 404)
    }
    ...
    if (!setting.is_enabled) {
      return json({
        ok: true,
        skipped: true,
        reason: 'Report delivery scheduler is disabled',
      })
    }

    const { data: dueRows, error: dueError } = await supabase
      .from('v_report_delivery_candidates')
      .select('*')
    ...
    if (dueError) {
      return json({
        ok: false,
        error: 'Could not read report delivery candidates',
    ...
    for (const sub of candidates) {
      ...
      const { data: runRow } = await supabase
        ...
        .insert({
          ...
          started_at: nowIso,
          payload: {
            report_preset_id: sub.report_preset_id || null,
            filter_payload: sub.filter_payload || {},
          },
          created_at: nowIso,
          updated_at: nowIso,
        })
      ...
      try {
        ...
        const csvName = sub.include_csv
          ? `${String(sub.report_kind || 'report')
              .replace(/[^a-z0-9]+/gi, '-')
        ...
        if (String(sub.delivery_channel || 'email') === 'in_app') {
          ...
          let recipientEmails = String(sub.recipient_email || '')
            .split(/[;,]/)
          ...
          if (!recipientEmails.length) {
            throw new Error('No email recipient is configured for this report subscription.')
          }
    ...
    await supabase
      ...
      .update({
        ...
        last_dispatch_notes: candidates.length
          ? `Processed ${candidates.length} due report subscription(s).`
          : 'No report subscriptions were due.',
        updated_at: nowIso,
      })
      .eq('id', setting.id)
