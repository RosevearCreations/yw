...
serve(async (req) => {
  ...
  try {
    ...
    if (!(await hasModuleAccess(supabase, actorProfile, 'safety', 'view'))) return Response.json({ ok:false, error:'Safety / OHSA module view access is required.', module_key:'safety', required_access:'view' }, { status:403, headers:corsHeaders });
