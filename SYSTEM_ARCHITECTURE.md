...
### Current frontend workflow additions
- shared offline action outbox for account/admin actions
- side-by-side conflict review panels in Settings and Admin
- diagnostics banner with per-module startup timing details

## Backend
Supabase Auth, Postgres, Storage, Edge Functions.

## Current architecture notes
- latest schema remains current through migration `055_storage_onboarding_identity_change_and_bootstrap.sql`
- current pass adds no new SQL objects; it improves conflict resolution visibility and troubleshooting in the frontend shell
- duplicate failure points continue to be reduced by keeping evidence metadata and identity workflow state database-first
