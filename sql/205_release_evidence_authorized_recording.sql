...
-- Schema 205 — Release-source evidence authorized recording authority.
--
-- Build 217 creates an exact-main candidate and Build 219 performs completed-run verification.
-- This migration closes the remaining database recording bypass: a service-role caller must not be
-- able to raw-insert release evidence or invoke the legacy recorder with a hand-supplied `passed`
-- workflow status. Verified release evidence is now traceable to one repository/run/attempt/path,
-- one verified-payload digest, the exact current schema, and a recent final-verification timestamp.
--
-- This migration does not create release evidence, change GitHub branch protection, run staging
-- acceptance, enable Finance/provider mutation, deploy Production, or promote main.
...
-- Preserve historical rows but do not let a pre-contract `passed` row remain release-green without
-- the exact traceability required by the authorized recording contract.
update public.it_release_source_evidence
set
  workflow_status='unknown',
  evidence_note=concat_ws(' ',nullif(evidence_note,''),
    'Schema 205 demoted legacy passed evidence because current verified-recording traceability was absent.')
where workflow_status='passed'
  and (
    repository is distinct from 'RosevearCreations/yw'
    or source_branch is distinct from 'main'
    or workflow_run_id is null or workflow_run_id<=0
    or workflow_run_attempt is null or workflow_run_attempt<=0
...
-- A rerun attempt is distinct evidence. Replace the Schema 166 uniqueness contract, which could not
-- distinguish attempts for the same run id.
...
-- Force authoritative writes through the narrow recorder. The service role retains read access.
...
-- The legacy recorder accepted caller-controlled workflow status and branch-policy values. Retain the
-- historical function definition for migration audit compatibility but remove its runtime authority.
...
begin
  if btrim(coalesce(p_project_ref,''))<>'jmqvkgiqlimdhcofwkxr' then
    raise exception 'Release evidence target project does not match the registered YardWeasels Production project.' using errcode='22023';
  end if;
  if not exists(
    select 1 from public.it_runtime_environment_authorities a
    where a.project_ref='jmqvkgiqlimdhcofwkxr'
      and a.environment_class='production'
      and a.staging_acceptance_mutation_allowed=false
  ) then
  ...
  if coalesce(p_workflow_run_id,0)<=0 or coalesce(p_workflow_run_attempt,0)<=0 then
    raise exception 'Positive workflow run id and run attempt are required.' using errcode='22023';
  end if;
  if btrim(coalesce(p_workflow_name,''))<>'YWI source and staging checks' then
    raise exception 'Release evidence must use the canonical YWI workflow.' using errcode='22023';
  end if;
  if btrim(coalesce(p_workflow_path,''))<>'.github/workflows/staging-browser-integration.yml' then
    raise exception 'Release evidence must use the canonical workflow path.' using errcode='22023';
  end if;
  if p_branch_protection_reported is distinct from true then
    raise exception 'Current GitHub main protection must be reported true before recording.' using errcode='22023';
  end if;
  if p_verification_contract_version is distinct from 1 then
    raise exception 'Unsupported release evidence verification contract version.' using errcode='22023';
  end if;
  ...
  if v_drift is distinct from 'current'
    ...
    raise exception 'Production schema must exactly match the verified source schema before recording release evidence.' using errcode='55000';
  end if;

  insert into public.it_release_source_evidence(
    repository,source_branch,source_sha,workflow_run_id,workflow_run_attempt,workflow_name,workflow_path,
    workflow_status,schema_version,branch_protection_reported,branch_policy_verified,
    verification_contract_version,verified_payload_sha256,verified_at,evidence_note,recorded_by_profile_id
  ) values (
    'RosevearCreations/yw','main',v_sha,p_workflow_run_id,p_workflow_run_attempt,
    'YWI source and staging checks','.github/workflows/staging-browser-integration.yml',
    'passed',p_schema_version,true,false,1,v_digest,p_verified_at,
    nullif(btrim(coalesce(p_evidence_note,'')),''),p_recorded_by_profile_id
  )
...
as $$
  select 'release_evidence_direct_service_write_blocked',
    ...
    'Service role may read release evidence but cannot bypass the validated recorder with raw table writes.'
  union all select 'legacy_release_recorder_disabled',
    ...
    'The legacy caller-controlled release evidence recorder is no longer executable by service_role.'
  union all select 'verified_release_recorder_service_only',
    ...
    'Only service_role may execute the verified release-source recording RPC.'
  union all select 'passed_release_traceability_required',
    case when not exists(
      select 1 from public.it_release_source_evidence e
      where e.workflow_status='passed' and (
        e.repository is distinct from 'RosevearCreations/yw'
        or e.source_branch is distinct from 'main'
        or coalesce(e.workflow_run_id,0)<=0
        or coalesce(e.workflow_run_attempt,0)<=0
        or e.workflow_name is distinct from 'YWI source and staging checks'
        or e.workflow_path is distinct from '.github/workflows/staging-browser-integration.yml'
        or e.branch_protection_reported is distinct from true
        or e.branch_policy_verified is distinct from false
        or e.verification_contract_version is distinct from 1
        or e.verified_payload_sha256 is null or e.verified_payload_sha256 !~ '^[0-9a-f]{64}$'
        or e.verified_at is null
      )
    ) then 'passed' else 'failed' end,
    'Every passed release-source row satisfies the canonical repository/run/attempt/path/digest traceability contract.'
...
insert into public.it_readiness_check_registry(
  ...
  'release_source_recording_authority','Release','Verified release-source evidence recording authority','critical',
  'Keep raw release-evidence writes and the legacy recorder disabled. Record only a freshly re-verified exact-main payload through ywi_record_verified_release_source_evidence after Production schema parity is exact.',
...
insert into public.admin_scorecard_progress_rails(
  ...
  'schema205_release_evidence_authorized_recording','admin','Verified release-source evidence authorized recording',
  'active',80,8,10,
  'Verify raw writes and the legacy recorder are locked, verified recording remains service-private, exact-source gates pass, and Production promotion remains separate and deliberate.',
  ...
  jsonb_build_object(
    ...
    'finance_mutation',false,'payment_provider_mutation',false,'staging_execution',false,
...
insert into public.it_scorecard_rail_resolution_contracts(
  ...
  'Close only after schema/source/release proof. This technical rail does not record evidence, verify GitHub policy, promote Production, or close human/external/business rails.',205
)
on conflict(rail_key) do update set
...
insert into public.app_schema_versions(
  ...
  'Forces passed release-source evidence through a traceable verified-recording RPC and locks legacy/raw write bypasses.',
  'applied',now(),'schema205',
  'No release evidence is created by the migration. No GitHub mutation, staging execution, Finance/provider enablement, business-rail closure, or Production promotion.',
...
select
  205 as expected_schema_version,
  ...
  case
    when coalesce(max(schema_version) filter (where status='applied'),0)=205
