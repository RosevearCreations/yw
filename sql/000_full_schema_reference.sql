...
insert into public.app_roadmap_action_steps (step_key, step_batch, step_number, step_area, step_title, step_status, priority, source_document, target_route_hint, acceptance_check, completion_note, risk_if_skipped, sort_order, metadata, checked_at) values
('schema128_done_01','completed_20',1,'schema','Added schema 128 execution queues','done','high','DEVELOPMENT_ROADMAP.md','#admin','Migration and full schema include schema 128.','DB-visible queues now track accounting, equipment, SEO, and fallback depth.','Roadmap stays Markdown-only.',1,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_done_02','completed_20',2,'admin','Exposed schema 128 queues in Admin readiness','done','high','DEVELOPMENT_ROADMAP.md','#admin','admin-directory and admin-ui reference schema 128 views.','Operators can see the new queues.','New rows stay hidden.',2,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_done_03','completed_20',3,'cleanup','Archived current Markdown and retired test_write files','done','high','DEVELOPMENT_ROADMAP.md','archive','Smoke archive hygiene passes.','Root stays clean.','Stale files keep breaking smoke.',3,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_done_04','completed_20',4,'cache','Updated cache marker to 2026-06-03a','done','high','DEVELOPMENT_ROADMAP.md','/','index and service worker markers match.','Stale browser issues are clearer.','Old cache can mask repairs.',4,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_done_05','completed_20',5,'docs','Updated active Markdown and added schema 128 docs','done','high','DEVELOPMENT_ROADMAP.md','docs','Docs reflect schema 128.','Future pass has context.','Docs drift from code.',5,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_next_01','next_20',1,'accounting','Build payment apply/reverse/adjust/refund UI actions','planned','high','DEVELOPMENT_ROADMAP.md','#admin','Payment actions move from registry to working buttons.','Cash application becomes usable.','Cash remains manual.',101,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_next_02','next_20',2,'accounting','Build bank CSV preview and staging screen','planned','high','DEVELOPMENT_ROADMAP.md','#admin','Header/date/duplicate/sign validation appears before staging.','Bad imports are blocked.','Bad data can enter.',102,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_next_03','next_20',3,'accounting','Build reconciliation match/split/undo/signoff workflow','planned','high','DEVELOPMENT_ROADMAP.md','#admin','Reviewer can match, split, undo, note, and sign off.','Reconciliation is auditable.','Rows remain unmatched.',103,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_next_04','next_20',4,'equipment','Add real camera scan and DB accessory templates','planned','high','DEVELOPMENT_ROADMAP.md','#jobs','Scanner and templates replace manual-only flow.','Field checkout is faster and clearer.','Manual entry stays slow.',104,'{"build":"2026-06-03a","schema":128}'::jsonb,now()),
('schema128_next_05','next_20',5,'seo','Generate sitemap/robots and add broken-link/image-alt/JSON-LD smoke checks','planned','medium','DEVELOPMENT_ROADMAP.md','/','Technical SEO files and smoke checks exist.','Public publish is safer.','SEO files drift.',105,'{"build":"2026-06-03a","schema":128}'::jsonb,now())
on conflict (step_key) do update set step_batch=excluded.step_batch, step_number=excluded.step_number, step_area=excluded.step_area, step_title=excluded.step_title, step_status=excluded.step_status, priority=excluded.priority, source_document=excluded.source_document, target_route_hint=excluded.target_route_hint, acceptance_check=excluded.acceptance_check, completion_note=excluded.completion_note, risk_if_skipped=excluded.risk_if_skipped, sort_order=excluded.sort_order, metadata=excluded.metadata, checked_at=excluded.checked_at, updated_at=now();
