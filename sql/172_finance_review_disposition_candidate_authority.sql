...
-- Build 2026-09-02d
-- Moves completed-job accounting disposition into Finance ownership.
-- Human Finance approval/rejection is required before candidate generation.
-- Candidate values are derived from canonical work-order/completion records; browser-supplied financial truth is rejected.
-- This release creates draft review candidates only. It does not post invoices/journals, mutate payment providers, or write back into Jobs completion state.
...
create table if not exists public.finance_job_completion_review_dispositions (
  ...
  constraint finance_job_completion_review_dispositions_approval_chk
    check (
      ...
      or
      (disposition_status='rejected' and candidate_generation_status='not_eligible')
    )
);
...
-- One invoice and one journal candidate maximum per Finance intake under the Schema 172 authority.
...
-- Guard legacy/browser candidate creation for completion reviews that have entered the Finance consumer pipeline.
...
begin
  ...
  -- Preserve unrelated historic candidate workflows. Once a completion review has Finance intake,
  -- however, Schema 172 is the only candidate authority.
  ...
  from public.job_completion_reviews
  where id=v_intake.completion_review_id;
  ...
  where id=v_review.work_order_id;
  if v_work_order.id is null then
    raise exception 'Canonical work order not found for Finance candidate generation.';
  end if;
  ...
  if new.candidate_status <> 'draft' then
    raise exception 'Schema 172 creates draft invoice candidates only.';
  end if;
...
begin
  ...
  if v_review.id is null then
    raise exception 'Canonical completion review not found for Finance candidate generation.';
  end if;
  ...
  if new.candidate_status <> 'draft' then
    raise exception 'Schema 172 creates draft journal candidates only.';
  end if;
...
begin
  if coalesce(old.payload->>'candidate_authority','')='schema172_finance_review' then
    ...
    if new.candidate_status='posted' then
      raise exception 'Schema 172 does not authorize candidate posting.';
    end if;
  end if;
  return new;
end;
...
create trigger trg_guard_finance_completion_invoice_candidate
before insert on public.job_invoice_candidates
...
create trigger trg_guard_finance_completion_journal_candidate
before insert on public.job_journal_candidates
...
create trigger trg_guard_schema172_invoice_candidate_posting
before update on public.job_invoice_candidates
...
create trigger trg_guard_schema172_journal_candidate_posting
before update on public.job_journal_candidates
for each row execute function public.ywi_guard_schema172_candidate_posting();
...
begin
  if v_status not in ('approved','rejected') then
    raise exception 'disposition must be approved or rejected';
  end if;
  if length(v_reason) < 3 then
    raise exception 'A Finance disposition reason is required.';
  end if;
  ...
  if v_intake.intake_status <> 'finance_review_queued' then
    raise exception 'Finance completion intake is not awaiting human review.';
  end if;
  ...
  if not exists(
    ...
    raise exception 'Finance intake source event no longer resolves to canonical jobs.job_completed.';
  end if;

  if not exists(
    select 1 from public.job_completion_reviews r
    where r.id=v_intake.completion_review_id and r.job_id=v_intake.job_id
      and r.review_status in ('approved','ready_for_accounting','posted')
      and r.accounting_ready=true
  ) then
    raise exception 'Canonical completion review is not accounting-ready.';
  end if;
  ...
  set intake_status='processed',updated_at=now()
  where id=v_intake.id;

  update public.job_completion_accounting_events
  set event_status='completed',completed_at=coalesce(completed_at,now()),
      payload=payload || jsonb_build_object(
        'schema172_disposition_id',v_id,
        'finance_disposition',v_status,
        'finance_reviewed_by_profile_id',p_actor_profile_id,
        'candidate_generation_authorized',v_status='approved'
      )
  where id=v_intake.finance_queue_event_id
    and accounting_action='queue_review'
    and event_status='queued';

  return query
  select d.id,d.intake_id,d.disposition_status,d.candidate_generation_status
...
begin
  ...
  if v_disposition.candidate_generation_status='blocked' then
    raise exception 'Candidate generation is blocked for this Finance disposition.';
  end if;

  if v_disposition.candidate_generation_status='generated'
     and v_disposition.invoice_candidate_id is not null
     and v_disposition.journal_candidate_id is not null then
  ...
  v_payload := jsonb_build_object(
    ...
    'completion_review_id',v_review.id,
    'canonical_amount_source','work_orders',
  ...
  from public.job_invoice_candidates
  where payload->>'candidate_authority'='schema172_finance_review'
    and payload->>'finance_intake_id'=v_intake.id::text
  limit 1;
  ...
  if v_invoice_id is null then
    insert into public.job_invoice_candidates(
      ...
      v_work_order.client_id,v_work_order.client_site_id,
      coalesce(v_work_order.subtotal,0),coalesce(v_work_order.tax_total,0),coalesce(v_work_order.total_amount,0),
      'Draft invoice candidate generated after approved Finance completion review. Posting remains unauthorized.',
      v_payload,p_actor_profile_id
    ) returning id into v_invoice_id;
  end if;

  select id into v_journal_id
  from public.job_journal_candidates
  where payload->>'candidate_authority'='schema172_finance_review'
    and payload->>'finance_intake_id'=v_intake.id::text
  limit 1;
  ...
  if v_journal_id is null then
    insert into public.job_journal_candidates(
      ...
      jsonb_build_object(
        'revenue_total',coalesce(v_review.revenue_total,0),
        'cost_total',coalesce(v_review.cost_total,0),
        'profit_total',coalesce(v_review.profit_total,0),
        'margin_percent',coalesce(v_review.margin_percent,0),
...
select
  ...
  r.review_status as completion_review_status,
  r.completion_date,
  r.accounting_ready,
  r.revenue_total,
  r.cost_total,
  ...
  coalesce(p.full_name,p.email,'') as reviewed_by_name,
  d.reviewed_at,
  ...
  jc.candidate_status as journal_candidate_status,
  i.created_at as queued_at
...
select
  ...
  min(i.created_at) filter(where d.id is null and i.intake_status='finance_review_queued') as oldest_awaiting_disposition_at,
...
set search_path=public,pg_temp
as $$
  select 'schema172_disposition_rls',
    case when c.relrowsecurity then 'passed' else 'failed' end,
  ...
  select 'schema172_service_role_rpc_boundary',
    case when not exists(
      ...
      where routine_schema='public'
        and routine_name in ('ywi_finance_dispose_job_completion_review','ywi_finance_generate_job_completion_candidates')
  ...
  select 'schema172_candidate_idempotency',
    case when to_regclass('public.uq_job_invoice_candidates_schema172_finance_intake') is not null
      and to_regclass('public.uq_job_journal_candidates_schema172_finance_intake') is not null
  ...
  select 'schema172_candidate_db_guards',
    case when (
      ...
      where t.tgname in ('trg_guard_finance_completion_invoice_candidate','trg_guard_finance_completion_journal_candidate','trg_guard_schema172_invoice_candidate_posting','trg_guard_schema172_journal_candidate_posting')
  ...
  select 'schema172_no_jobs_writeback',
    case when not exists(
      ...
      where n.nspname='public'
        and p.proname in ('ywi_finance_dispose_job_completion_review','ywi_finance_generate_job_completion_candidates')
  ...
  select 'schema172_no_posting_or_provider_mutation',
    case when not exists(
      select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
      where n.nspname='public'
        and p.proname='ywi_finance_generate_job_completion_candidates'
        and (
          pg_get_functiondef(p.oid) ~* E'insert\\s+into\\s+public\\.(ar_invoices|gl_batches|gl_entries|payments)'
          or pg_get_functiondef(p.oid) ilike '%stripe%'
          or pg_get_functiondef(p.oid) ilike '%paypal%'
        )
    ) then 'passed' else 'failed' end,
...
-- Extend the I.T. consumer health feed so Admin > I.T. remains the operational visibility surface.
...
with source_events as (
  ...
  select
    count(*) filter (where accounting_action='queue_review' and event_status='queued' and payload->>'consumer'='finance_job_completion_consumer')::bigint as queued_accounting_review_count,
  ...
  select se.*,i.*,aq.* from source_events se cross join intake i cross join accounting_queue aq
), execution as (
  select * from public.v_finance_job_completion_execution_status
), review_status as (
  select * from public.v_finance_job_completion_review_status
)
select
  'finance_completion_unconsumed'::text as check_key,'Unconsumed Jobs completion events'::text as check_title,
  ...
  case when unconsumed_event_count=0 then 'Every canonical jobs.job_completed event has a Finance intake row.' when oldest_unconsumed_at < now()-interval '1 day' then 'At least one canonical completed-job event has remained unconsumed for more than 24 hours.' else 'Finance has completed-job events waiting for service-owned intake.' end::text as details,
  'Run the bounded Finance consumer only through the Schema 171 service-role runner after execution readiness is green.'::text as next_action_hint,now() as checked_at
from facts
union all select
  'finance_completion_failed_intake','Failed Finance completion intake',case when failed_intake_count=0 then 'passed' else 'failed' end,
  failed_intake_count,intake_count,null::timestamptz,
  case when failed_intake_count=0 then 'No Finance completed-job intake rows are in failed state.' else 'One or more Finance completed-job intake rows require investigation before downstream automation.' end,
  'Inspect the private Finance intake and source event under service-role tooling; do not repair Jobs state from Finance.',now()
from facts
union all select
  'finance_completion_review_queue','Finance accounting review queue',case when queued_accounting_review_count=0 then 'passed' when oldest_accounting_review_at < now()-interval '7 days' then 'failed' else 'warning' end,
  queued_accounting_review_count,finance_review_queued_count,oldest_accounting_review_at,
  case when queued_accounting_review_count=0 then 'No Finance review items created by the completed-job consumer are currently queued.' when oldest_accounting_review_at < now()-interval '7 days' then 'A completed-job Finance review has remained queued for more than seven days.' else 'Completed-job Finance reviews are queued for human/accounting handling.' end,
  'Review queued accounting work in Finance. Candidate generation requires Schema 172 approval.',now()
from facts
union all select
  'finance_completion_pipeline_balance','Jobs completion / Finance intake balance',
  ...
  intake_count,source_event_count,oldest_queued_intake_at,
  'Finance intake must never exceed canonical source events, and every intake row must resolve to a declared intake status.',
  'Repair consumer-state integrity before releasing dependent accounting automation.',now()
from facts
union all select
  'finance_completion_execution_readiness','Controlled Finance consumer execution',e.check_status,e.unconsumed_event_count,coalesce(e.last_consumed_count,0)::bigint,e.last_started_at,e.details,
  'Execution is service-role-only. Admin I.T. remains read-only and must not expose a browser execute or retry control.',now()
from execution e
union all select
  'finance_completion_retry_state','Finance consumer retry state',case when e.exhausted_failure_count > 0 then 'failed' when e.open_failure_count > 0 then 'warning' else 'passed' end,
  (e.open_failure_count + e.exhausted_failure_count)::bigint,e.failure_record_count,e.next_retry_at,
  case when e.exhausted_failure_count > 0 then 'At least one event reached the three-attempt retry ceiling.' when e.retry_ready_count > 0 then 'At least one failed event is eligible for an explicit service-role retry run.' when e.open_failure_count > 0 then 'Failed events are inside their retry backoff window.' else 'No unresolved Finance consumer retry state exists.' end,
  'Investigate exhausted failures before any dependent accounting automation is released.',now()
from execution e
union all select
  'finance_completion_human_disposition','Finance completion human disposition',
  ...
  'Use the Finance workspace to approve or reject the completion review. I.T. is visibility-only.',now()
from review_status r
union all select
  'finance_completion_candidate_generation','Finance completion candidate generation',
  ...
  'Generate draft candidates from Finance only after approval. Posting/payment/provider mutation remains out of scope.',now()
from review_status r;
...
insert into public.it_readiness_check_registry(
  ...
  'finance_completion_review_authority','Architecture','Finance completion review disposition and candidate authority','critical',
  'Require a human Finance approve/reject disposition before candidate generation. Generate draft candidates only; do not bypass canonical amount guards or posting/provider locks.',
...
insert into public.admin_scorecard_progress_rails(
  ...
  'schema172_finance_review_disposition','architecture','Finance completion review disposition and candidate authority','active',90,8,9,
  'Merge exact green Schema 172 source, prove exact main, then apply/verify Finance approve-reject and draft-candidate authority without fabricating live business events.',
  'Finance / I.T. / Architecture',92,
  '{"build":"2026-09-02d","schema":172,"human_disposition":true,"canonical_amounts":true,"draft_candidates_only":true,"posting":false,"provider_mutation":false,"jobs_writeback":false,"it_visibility":true}'::jsonb
)
on conflict(rail_key) do update set
  rail_area=excluded.rail_area,rail_title=excluded.rail_title,rail_status=excluded.rail_status,progress_percent=excluded.progress_percent,
  current_value=excluded.current_value,target_value=excluded.target_value,next_action_hint=excluded.next_action_hint,owner_hint=excluded.owner_hint,
...
insert into public.app_schema_versions(
  ...
  172,'172_finance_review_disposition_candidate_authority','172_finance_review_disposition_candidate_authority.sql','2026-09-02d',
  ...
  'applied',
  'Finance owns approve/reject and candidate generation. Jobs cannot supply candidate amounts. Draft candidates cannot be posted under Schema 172. No payment/provider mutation, Jobs writeback, fifth module or Production promotion is introduced.'
)
on conflict(schema_version) do update set
  migration_key=excluded.migration_key,schema_name=excluded.schema_name,release_label=excluded.release_label,description=excluded.description,status=excluded.status,notes=excluded.notes,applied_at=now();
...
select 172::int as expected_schema_version,
  ...
  case when coalesce(max(schema_version) filter(where status='applied'),0)>=172
    ...
    else 'Live database is behind the deployed app. Apply migrations through schema 172.' end as message,
  now() as checked_at
