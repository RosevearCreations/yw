...
-- Schema 199 — I.T. next-safe-action authority.
-- This is prioritization only. It never performs staging mutation, closes a business rail,
-- changes Supabase Auth, enables Finance posting/provider mutation, or promotes Production.
...
select
  ...
  case
    when t.source_kind='staging_acceptance' and t.todo_status='ready'
      then 'Candidate only. Before any mutation, re-verify the dedicated non-production staging environment guard and use labelled disposable/test data. Human signoff remains required.'
...
as $$
  ...
  union all select 'ready_staging_only_safe_candidates',
    ...
    'Only technically ready staging-acceptance items can be marked as safe candidates, and they still require the runtime environment guard.'
  ...
  union all select 'service_private_authority',
    ...
    then 'passed' else 'failed' end,
    'The next-safe-action authority is service-role-only.';
$$;
...
values(
  'next_safe_action_authority','I.T.','Next safe action authority','warning',
  'Use current readiness priority: ready staging candidates first, external controls separately, content/provider pending, accounting blocked. Never bypass environment or Finance/provider guards.',
...
insert into public.admin_scorecard_progress_rails(
  ...
  'schema199_next_safe_action_authority','admin','I.T. next safe action authority','active',80,8,10,
  'Verify Schema 199 assertions, exact current To-Do parity, safe prioritization, source/browser gates, release evidence, and cleanup. Do not auto-close human/external rails.',
  'Admin / I.T.',119,jsonb_build_object(
    'schema',199,'staging_mutation',false,'business_rail_auto_close',false,'auth_setting_mutation',false,
...
insert into public.it_scorecard_rail_resolution_contracts(
  ...
  'schema199_next_safe_action_authority','build_acceptance',false,false,false,
  'Close only after schema/source/browser/exact-release proof. Prioritized business and external actions remain independently open.',199
)
on conflict(rail_key) do update set
...
values(
  199,'199_next_safe_action_authority','Adds service-private, fail-closed prioritization of current I.T. actions without executing or auto-closing them.',
  'applied',now(),'schema199','No staging/Auth/Finance/provider mutation and no human/external rail closure.',
  '199_next_safe_action_authority.sql','schema199'
)
