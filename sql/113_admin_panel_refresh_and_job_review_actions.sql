...
-- Low-risk tracking migration for the 2026-05-17b pass. This adds operational tracking
-- around the new UI behavior without changing core accounting or job table structure.
...
insert into public.admin_panel_refresh_preferences (
  panel_key, panel_title, edge_scope, preferred_timeout_ms, last_refresh_status, notes
)
values
  ('staff_directory', 'Staff Directory and Access', 'people', 12000, 'ready', 'Refresh Staff Only uses the people fast path instead of reloading all Admin data.'),
  ('jobs_operations', 'Jobs and Operations', 'operations', 12000, 'ready', 'Refresh Jobs Only uses the operations fast path and keeps the current Jobs filter/page.'),
  ('health_schema', 'App Health and Schema Center', 'health', 12000, 'ready', 'Health panel can be refreshed through a narrow health/command-center scope.'),
  ('accounting_close', 'Guided Close Center', 'accounting', 12000, 'ready', 'Close center can refresh close blockers without reloading Staff Directory data.'),
  ('reporting', 'Reporting', 'reporting', 20000, 'ready', 'Reports remain lazy-loaded and use the reporting fast path.')
...
insert into public.app_frontend_quality_gates (
  gate_key, gate_area, gate_title, gate_status, route_hint, command_hint, failure_hint, checked_at, sort_order
)
values
  ('admin_panel_refresh_people_scope', 'Admin UX', 'Staff panel refresh uses people-only Edge scope', 'passed', '#admin', 'Open Admin > People and Access, press Refresh Staff Only, and confirm other sections are not reloaded.', 'Redeploy admin-directory and clear service worker cache if the button still reloads the full Admin manager.', now(), 310),
  ('admin_panel_refresh_operations_scope', 'Admin UX', 'Jobs panel refresh uses operations-only Edge scope', 'passed', '#admin', 'Open Admin > Jobs and Operations, press Refresh Jobs Only, and confirm Jobs paging remains intact.', 'Redeploy admin-directory and clear service worker cache if Jobs reload is slow or resets filters.', now(), 320),
  ('admin_jobs_review_table_mobile', 'Mobile UX', 'Jobs review table has compact mobile row actions', 'passed', '#admin', 'On a phone-width viewport, confirm Open, Complete, Cancel, and Add Note stack cleanly.', 'Check style.css admin-row-actions and admin-jobs-review-wrap rules if actions overflow.', now(), 330),
  ('admin_job_action_handlers', 'Admin Actions', 'Admin Manage supports job status and note actions', 'passed', '#admin', 'Use a safe test job to add a note, then review job_comments and site activity.', 'Redeploy admin-manage if row actions return unsupported job action.', now(), 340)
...
select
  113 as repo_schema_version,
  coalesce(max(schema_version) filter (where status = 'applied'), 0)::int as latest_applied_schema_version,
  case when coalesce(max(schema_version) filter (where status = 'applied'), 0) >= 113 then 'current' else 'behind' end as drift_status,
  case when coalesce(max(schema_version) filter (where status = 'applied'), 0) >= 113
