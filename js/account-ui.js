...
(function () {
  ...
  function render(state) {
    ...
    if (els.panel) {
      els.panel.hidden = !isAuthenticated;
    }
    ...
    if (els.email) {
      els.email.value = current.profile?.email || current.user?.email || '';
    }

    if (els.role) {
      els.role.value = access.roleLabel || role;
    }
  }
  ...
  async function onSavePassword() {
    try {
      ...
      try {
        ...
        if (els.password) els.password.value = '';
        if (els.confirm) els.confirm.value = '';
        setSummary('Password updated successfully.');
      } finally {
        restore();
      }
    } catch (err) {
