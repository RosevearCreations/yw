...
(function () {
  ...
  const MODULE_MANIFEST = Object.freeze({
    safety: Object.freeze({
      key: 'safety', label: 'Safety / OHSA', version: CONTRACT_VERSION,
      scripts: Object.freeze(['/js/hse-ops-ui.js','/js/logbook-ui.js','/js/reports-ui.js','/js/forms-toolbox.js','/js/forms-ppe.js','/js/forms-firstaid.js','/js/forms-incident.js','/js/forms-inspection.js','/js/forms-drill.js']),
      coreDependencies: SHARED_CORE_DEPENDENCIES
    }),
    finance: Object.freeze({
      key: 'finance', label: 'Finance', version: CONTRACT_VERSION,
    ...
    jobs: Object.freeze({
      key: 'jobs', label: 'Jobs', version: CONTRACT_VERSION,
  ...
  const state = { syncing:false, queued:false, reloading:false, activeProfileId:null, loadedModules:new Set(), loadedScripts:new Set(), failedScripts:new Map(), lastSyncAt:0 };
  ...
  function normalizeScriptSrc(src) { try { return new URL(src, window.location.origin).pathname; } catch { return String(src || '').split('?')[0]; } }
  function existingScript(src) { const target=normalizeScriptSrc(src); return [...document.scripts].find((script)=>normalizeScriptSrc(script.src || script.getAttribute('src') || '')===target) || null; }
  ...
  function loadScript(src,moduleKey) {
    ...
    return new Promise((resolve,reject)=>{
      ...
      script.onerror=()=>{const error=new Error(`Unable to load ${moduleKey} module script: ${src}`);state.failedScripts.set(normalized,error.message);reject(error);};
      document.head.appendChild(script);
    });
  }
  ...
  function initializeLoadedFactories(){window.initFormModules?.();window.initProtectedModules?.();window.seedAllTables?.();window.YWIModuleNav?.sync?.();}

  async function syncForCurrentAccess(){
    if(state.syncing){state.queued=true;return false;}
    state.syncing=true;state.queued=false;
    try{
      const stateNow=authState();
      ...
      return false;
    }finally{state.syncing=false;if(state.queued && !state.reloading) queueMicrotask(()=>syncForCurrentAccess());}
  }

  function getRuntimeState(){return {build:BUILD,contractVersion:CONTRACT_VERSION,activeProfileId:state.activeProfileId,loadedModules:[...state.loadedModules],loadedScripts:[...state.loadedScripts],failedScripts:Object.fromEntries(state.failedScripts),lastSyncAt:state.lastSyncAt,reloading:state.reloading};}
