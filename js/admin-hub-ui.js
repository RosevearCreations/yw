...
(function () {
  ...
  function createController(section, instance, deferredScopes) {
    ...
    function loadBuild344ChangeOrdersExtras() {
      window.YWIChangeOrdersExtrasUI?.mount?.({ api:window.YWIAPI });
    }

    function loadBuild345QualityControl() {
      window.YWIQualityControlUI?.mount?.({ api:window.YWIAPI });
    }

    function loadBuild347UniversalActivityTimeline() {
    ...
    function loadBuild350OwnerManagement() {
      if (!can('manage')) return;
      window.YWIOwnerManagementCommandUI?.mount?.({ api:window.YWIAPI });
    }
