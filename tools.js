...
function normalizeFromCatalog(row) {
  ...
  const rawName = row.name || source.item_name_suggested || source.name || source.display_name || source.title || source.example_image_file || 'Tool';
  return Object.assign({}, source, {
    ...
    name: rawName,
    category: row.category || source.category || source.tool_category || '',
...
async function loadJsonFallback(request) {
  try {
    const response = await fetch(new URL('/data/toolshed/toolshed_items_master.json', request.url).toString(), { cf: { cacheTtl: 0, cacheEverything: false } });
    ...
    return {
      items: items.map((item, index) => Object.assign({}, item, {
        id: item.id || item.item_group_key_strict || item.item_group_key_loose || item.slug || `${slugify(item.item_name_suggested || item.name || 'tool')}-${index + 1}`,
        slug: item.slug || slugify(item.item_name_suggested || item.name || item.example_image_file || 'tool'),
...
export async function onRequestGet(context) {
  ...
  if (db) {
    try {
      ...
      await captureRuntimeIncident(env, request, {
        ...
        incident_code: 'tool_d1_read_failed',
        severity: 'warning',
        message: 'Tools D1 read failed. Public JSON fallback was used.',
  ...
  if (!items.length) {
    ...
    if (fallback.items.length) {
      ...
      await captureRuntimeIncident(env, request, {
        incident_scope: 'public_catalog',
        incident_code: 'tool_json_fallback_failed',
        severity: 'error',
        message: 'Tools JSON fallback could not be loaded.',
