...
// Brief description: Returns the current authenticated session details and a small summary for the logged-in user.
...
export async function onRequestGet(context) {
  ...
  const allSessionsResult = await env.DB.prepare(`SELECT session_id, expires_at FROM sessions WHERE user_id = ?`).bind(Number(sessionUser.resolved_user_id || sessionUser.user_id || 0)).all();
  const sessions = Array.isArray(allSessionsResult?.results) ? allSessionsResult.results : [];
  return json({ ok: true, user: { user_id:Number(sessionUser.resolved_user_id || sessionUser.user_id || 0), email:sessionUser.email || "", display_name:sessionUser.display_name || "", role:sessionUser.role || "member", is_active:Number(sessionUser.is_active || 0), created_at:sessionUser.created_at || null, updated_at:sessionUser.updated_at || null }, session: { session_id:Number(sessionUser.session_id || 0), expires_at:sessionUser.expires_at || null, created_at:sessionUser.session_created_at || null, is_current:true }, session_summary: { total_sessions:sessions.length, active_sessions:sessions.filter((row) => typeof row?.expires_at === 'string').length } });
}
